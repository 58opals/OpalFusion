...
struct TransactionFinalizationProposalValidator {
    ...
    func validateLegacyInitializerShape() {
        let proposal = OpalFusion.Host.TransactionFinalizationProposal(
            serializedUnsignedTransaction: [0x01, 0x02, 0x03]
        )

        #expect(proposal.serializedUnsignedTransaction == [0x01, 0x02, 0x03])
    ...
    func validateOptionalRoundMetadata() {
        let proposal = OpalFusion.Host.TransactionFinalizationProposal(
            serializedUnsignedTransaction: [0xAA, 0xBB],
        ...
        #expect(proposal.serializedUnsignedTransaction == [0xAA, 0xBB])
