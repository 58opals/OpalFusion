...
struct TransactionFinalizationProposalValidator {
    ...
    func validateLegacyInitializerShape() {
        let proposal = OpalFusion.Host.TransactionFinalizationProposal(
            unsignedFusionTransactionBytes: [0x01, 0x02, 0x03]
        )

        #expect(proposal.unsignedFusionTransactionBytes == [0x01, 0x02, 0x03])
    ...
    func validateOptionalRoundMetadata() {
        let proposal = OpalFusion.Host.TransactionFinalizationProposal(
            unsignedFusionTransactionBytes: [0xAA, 0xBB],
        ...
        #expect(proposal.unsignedFusionTransactionBytes == [0xAA, 0xBB])
