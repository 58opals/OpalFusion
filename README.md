...
Status: the CashFusion engine is a pilot on `develop`; Mosaic is a draft specification with a deterministic in-process semantic core, validated role-seed-to-roster integration, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and reassembly, strict NIP-01 event validation, an authenticated NIP-44 boundary, BIP340 manifest-round and contributor pre-sign acknowledgement validation, replay and relay-publication guards, wallet-host contracts, and an internal bounded RFC 9474 authorization path, but no discovery, frozen Opal v0 role hash documents, complete manifest/BCH-signature/proof messages, fragment-to-authenticated-envelope integration, live transport, independently reviewed cryptographic deployment, or live runtime claim. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
