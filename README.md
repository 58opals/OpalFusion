...
Status: the CashFusion engine is a pilot on `develop`; Mosaic remains non-live. Its deterministic foundations include the attempt and runtime reducers, an internal ordered host coordinator, the chipnet-only Opal v0 conformance contracts, and the additive `Mosaic/0-opal-mainnet-alpha.4` contract profile. Alpha.4 freezes role hashing, complete manifests, exact 23-slot contributor-local material, salt and Pedersen openings, deterministic allocation of BCH's fixed ten-satoshi transaction overhead, two component-bound blind-authorization purposes under distinct attempt keys, dual-vector response sets, complete contributor acknowledgements, typed aggregate reassembly, authenticated control admission, component mailbox sequence zero, BCH-signature mailbox sequence one, canonical signature sets, and exact complete-transaction assembly. Its sealed admission ledger binds facts to one attempt, generation, material, round, roster, phase, sender sequence, one-time x-only sender identity, and one-time recipient mailbox. The paired internal runtime consumes the exact acknowledgement, signature, and complete-transaction facts only through previous-output-backed validation. Internal role-specific executors prove contributor reservation, local signing, signature publication, byte-exact commit, pre-sign release, and post-sign recovery ordering, plus conductor blind-authorization issuance, aggregate publication, anonymous collection, signature verification, and exact transaction assembly, all against injected non-network seams. A strict internal NIP-59 structural codec now creates fresh outbound wrapper material and validates a distinct signed wrapper, sender-signed seal, unsigned rumor, and one matching recipient tag without assigning a Mosaic application kind or runtime fact. Inbound validation cannot prove remote wrapper-key freshness, and caller-owned nonrecipient tags remain outside this structural layer. An internal `PostManifestRuntimeDriver` constructs a fresh paired runtime, selects exactly one role executor, and forwards one serialized stream of already-authenticated deliveries and source closure. The generic transport-facing `RuntimeSessionDriver` remains disabled for mainnet-alpha and no public Mosaic session exists. Production stateful lease-to-material ownership, cross-attempt secret persistence and erasure, authoritative app composition, Mosaic-specific gift-wrap mapping and padding, authenticated mailbox and Tor transport, durable crash recovery, independent review, real mainnet spending, and broadcast remain unavailable. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
...
- Current live CashFusion transport support, including the Tor SOCKS5 covert path, is macOS-only in this package. Mosaic has strict relay framing, structural NIP-59 rumor/seal/gift-wrap validation, and an injected Tor-only WebSocket capability boundary, but no Mosaic event mapping or concrete live connection implementation.
