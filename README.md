...
Status: the CashFusion engine is a pilot on `develop`; Mosaic remains non-live. Its deterministic foundations include the attempt and runtime reducers, an internal ordered host coordinator, the chipnet-only Opal v0 conformance contracts, and the additive `Mosaic/0-opal-mainnet-alpha.2` contract profile with a sealed admission-only ledger. Mainnet-alpha freezes role hashing, complete manifests, PlayerCommit, per-contributor authorization-response sets, the complete contributor acknowledgement set, typed aggregate reassembly, authenticated control admission, BCH signature sets, and exact complete-transaction assembly. Its ledger binds admitted material through transcript agreement to one attempt and generation, derives a zero sequence epoch for every roster sender, enforces per-sender replay and immediate externally validated phase synchronization, requires complete PlayerCommit and response-set collection with contributor-local blind-response finalization, restricts anonymous intake to the conductor with one-time sender and mailbox identities, requires exact acknowledgement-set publication, and absorbs terminal input. It explicitly cannot admit BCH signing or complete transactions and does not feed the runtime. Separate profile-contract tests prove previous-output-backed transaction assembly; the coordinator proves in-process release, signing, exact-commit, cancellation, and recovery-required ordering against injected seams. Mainnet-alpha remains deliberately runtime-disabled. Production commitment opening and lease-material construction, previous-output resolution in runtime composition, anonymous BCH-signature authorization, mailbox-to-runtime integration, concrete Tor networking, durable recovery, independent review, and all mainnet spending or broadcast remain unavailable. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
...
The package includes the protocol-neutral facade and internal Mosaic attempt, transcript, authorization, relay-validation, ordered runtime, and host-coordination foundations. The host coordinator drains reducer effects through the exact public Mosaic wallet contract, releases a late reservation before signing, refuses automatic release after signing may have begun, commits only an injected validated complete transaction, and exposes recovery-required outcomes when disposition is ambiguous. It also includes the additive `Mosaic/0-opal-mainnet-alpha.2` deterministic contract profile: mainnet identity and fee rules, role hashing, complete manifest, PlayerCommit, per-contributor authorization-response sets, a complete contributor acknowledgement set, strict typed aggregate reassembly, authenticated control admission, signature-set validation, and exact complete-transaction assembly with golden vectors. Its internal admission ledger binds those admitted documents, commitment and component sets, and anonymous components to one attempt, generation, roster, round, local role, a roster-derived zero sequence epoch, and an immediate externally validated phase successor. It requires local blind-response finalization before a contributor leaves wallet reservation and exact portable acknowledgement publication at the conductor, but it neither converts material into `RuntimeSession` input nor admits BCH signing. The internal runtime driver still accepts only `.opalV0`; mainnet-alpha rejects legacy aggregate host markers and has no public session, mailbox-to-runtime bridge, production host composition, durable recovery driver, transport route, or broadcast path. Commitment-opening/component linkage, previous-output resolution in runtime composition, anonymous BCH-signature authorization, concrete Tor integration, and independent review remain release blockers, so the new profile is not a live Mosaic or mainnet-readiness claim. `OpalFusion.Client.Session` remains the only live entry point and runs CashFusion.
