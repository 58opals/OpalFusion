...
Status: the CashFusion engine is a pilot on `develop`; Mosaic is a draft specification with a deterministic in-process semantic core, validated role-seed-to-roster integration, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and reassembly, strict NIP-01 event and relay-frame validation, an injected one-relay correlation session, an authenticated NIP-44 boundary, BIP340 manifest-round and contributor pre-sign acknowledgement validation, deterministic Opal v0 unsigned-transaction construction, locally derived transcript-root gating, a fail-closed contributor-local inclusion-validation boundary, replay and relay-publication guards, wallet-host contracts, an internal ordered runtime-session driver, and a bounded RFC 9474 authorization path. Mosaic still has no production commitment-opening/inclusion validator, discovery, frozen Opal v0 role hash documents, complete manifest/BCH-signature/proof messages, previous-output-backed wallet validation, fragment-to-authenticated-envelope integration, concrete Tor/WebSocket connection, production effect executor, independently reviewed cryptographic deployment, or live runtime claim. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
...
The package now includes the protocol-neutral facade plus an internal peer-local Mosaic semantic core, deterministic multi-peer simulator, validated role-seed-to-roster integration, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and terminal reassembly, strict NIP-01 event and relay-frame codecs, a close-once one-relay session over an injected Tor-only WebSocket capability, NIP-44 encrypted-event boundary, authenticated replay and strict Opal-v0 sequence guards, relay-publication quorum tracker, bounded RFC 9474 authorization execution, deterministic unsigned-transaction construction, locally derived transcript-root gating, a fail-closed contributor-local inclusion boundary, generation-bound wallet-host contracts, and an ordered post-admission runtime-session driver. The executable transcript binding and runtime driver accept only `.opalV0`; `.draft1` fails closed. Opal v0 role-commitment and role-seed hash documents, a production commitment-opening/component-linkage validator, previous-output-backed validation, complete manifests, BCH-signature messages, proof/blame payloads, fragment-to-authenticated-envelope integration, a concrete Tor/WebSocket connection, production effect execution, and a public runtime remain unresolved or unimplemented, and the cryptographic path has not completed independent review, so these surfaces do not make Mosaic runnable over a network. `OpalFusion.Client.Session` remains the live CashFusion entry point; `OpalFusion.Session` intentionally has no public initializer yet.
