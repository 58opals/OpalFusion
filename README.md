...
An internal contributor transport bridge now binds the reservation coordinator's ordered PlayerCommit, anonymous-component, pre-sign-acknowledgement, and local-signature callbacks to the control and anonymous publication paths for one exact lease-backed material value. It pins the local inbound control capability to that contributor's allocated roster event key, mints an attempt-bound route group that one fan-in may claim exactly once, and separates a nonblocking stop request from the lifecycle owner's termination wait so active work drains before terminal state. It does not start or supervise fan-in, automatically connect source loss to that stop boundary, allocate keys or routes, choose expiry values, perform semantic loopback, persist attempt state, or enable a live session.
...
The package includes the protocol-neutral facade and internal Mosaic attempt, transcript, authorization, relay-validation, ordered-runtime, and host-coordination foundations. Its additive `Mosaic/0-opal-mainnet-alpha.4` profile supplies deterministic mainnet contract evidence for 6–8 contributors, exact 23-slot lease material, grouped Pedersen balance, two purpose-separated authorization vectors, response finalization bound to one attempt/generation/material, strict authenticated admission, and conductor collection of component and BCH-signature mailbox deliveries. The profile explicitly accepts that authorization does not prove a disclosed component belongs to a contributor-tagged grouped commitment; tests pin one valid off-commitment vector and make no privacy or accountability claim. The internal alpha.4 runtime consumes the complete acknowledgement, signature-set, and complete-transaction facts through exact previous-output-backed validation. Its contributor executor invokes the refined host contract through byte-exact commit, and its conductor executor deterministically issues both authorization vectors, hands each locally constructed aggregate to an injected publisher, advances only after its later authenticated admission, verifies anonymous signatures against resolved previous outputs, and assembles the exact complete transaction. The post-manifest lifecycle ingress accepts only signed gift wraps, resolves their sole `p` identity against an immutable attempt-scoped recipient set, and applies only that sealed channel and key before its internal `PostManifestRuntimeDriver` derives the bootstrap context, authenticates and decrypts the event, privately routes the resulting typed delivery, and selects the roster-derived role without owning a live network task. The internal control-publication bridge converts its bootstrap-bound manifest, sealed contributor reservation/transcript validations, and coordinator-minted conductor publications into one zero-origin sender-global sequence, preclaims complete fragment ranges, signs canonical control envelopes, and creates one recipient-bound gift wrap for every roster identity before an injected whole-batch handoff. The material-bound anonymous-publication bridge separately converts the contributor executor's sealed 23-component and local-input-signature validations into sequence-zero and sequence-one gift wraps for the same one-time mailboxes, using purpose-distinct sender keys and exposing only recipient identities plus sealed wraps to its whole-batch handoff. A separate one-shot internal publisher validates the alpha.5 outer gift-wrap shape and validator-sealed manifest relay endpoint identifiers, attempts the exact same event on three opaque injected Tor-only relay routes supplied by the route owner, enforces two accepted acknowledgements, and closes every route without reconnect or fallback. The bounded multi-recipient fan-in validates one shared manifest relay selection, requires one control mailbox and permits conductor-only anonymous mailboxes up to the roster capacity, gives every mailbox three globally distinct injected connections and subscription identifiers, starts all subscriptions before consuming into one runtime, forwards every EVENT copy unchanged through a bounded serial ingress queue, ignores EOSE and NOTICE, and terminates if any source ends. The generic `RuntimeSessionDriver` still accepts only `.opalV0`, `OpalFusion.Session` remains unconstructible, and no live transport or broadcast is selected. Production recipient allocation, authenticated distribution and encrypted persistence, complete mailbox-route provisioning, stateful material ownership, app-authoritative previous-output and host composition, authenticated mailbox/Tor adapters, authoritative relay selection and endpoint-to-capability binding, durable duplicate merging and replay, reconnect, crash recovery, explicit broadcast approval, and deployment review remain release gates. `OpalFusion.Client.Session` remains the only live entry point and runs CashFusion.
