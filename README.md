...
The package now includes the protocol-neutral facade plus an internal peer-local Mosaic semantic core, deterministic multi-peer simulator, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and terminal reassembly, strict NIP-01 event codec, NIP-44 encrypted-event boundary, authenticated replay guards, relay-publication quorum tracker, bounded RFC 9474 authorization execution, and narrow wallet-host contracts. Complete manifests, BCH-signature messages, proof/blame payloads, fragment-to-authenticated-envelope integration, transport adapters, and live effects remain unresolved or unimplemented, and the cryptographic path has not completed independent review, so these surfaces do not make Mosaic runnable over a network. `OpalFusion.Client.Session` remains the live CashFusion entry point; `OpalFusion.Session` intentionally has no public initializer yet.
