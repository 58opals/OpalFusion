...
Status: the CashFusion engine is a pilot on `develop`; Mosaic remains non-live. Its deterministic foundations include the attempt and runtime reducers, an internal ordered host coordinator, the chipnet-only Opal v0 conformance contracts, and an additive mainnet-alpha contract profile with a sealed admission-only ledger. Mainnet-alpha freezes role hashing, complete manifests, PlayerCommit, typed aggregate reassembly, authenticated control admission, individual pre-sign acknowledgements, BCH signature sets, and exact complete-transaction assembly. Its ledger binds admitted material through transcript agreement to one attempt and generation, enforces explicit sequence baselines, per-sender replay, immediate externally validated phase synchronization, complete PlayerCommit collection, conductor-only anonymous intake with one-time sender and mailbox identities, conductor-local individual-acknowledgement collection, and terminal absorption. It explicitly cannot admit BCH signing or complete transactions and does not feed the runtime. Separate profile-contract tests prove previous-output-backed transaction assembly; the coordinator proves in-process release, signing, exact-commit, cancellation, and recovery-required ordering against injected seams. Mainnet-alpha remains deliberately runtime-disabled. Production commitment opening and lease-material construction, authorization-response and complete acknowledgement-set contracts, previous-output resolution, anonymous BCH-signature authorization, mailbox-to-runtime integration, concrete Tor networking, durable recovery, independent review, and all mainnet spending or broadcast remain unavailable. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
...
The package includes the protocol-neutral facade and internal Mosaic attempt, transcript, authorization, relay-validation, ordered runtime, and host-coordination foundations. The host coordinator drains reducer effects through the exact public Mosaic wallet contract, releases a late reservation before signing, refuses automatic release after signing may have begun, commits only an injected validated complete transaction, and exposes recovery-required outcomes when disposition is ambiguous. It also includes the additive `Mosaic/0-opal-mainnet-alpha.1` deterministic contract profile: mainnet identity and fee rules, role hashing, complete manifest, PlayerCommit, strict typed aggregate reassembly, authenticated control admission, individual pre-sign acknowledgement, signature-set validation, and exact complete-transaction assembly with golden vectors. Its internal admission ledger binds the admitted manifest, PlayerCommits, commitment and component sets, anonymous components, and individual acknowledgements to one attempt, generation, roster, round, local role, explicit per-sender sequence baseline, and immediate externally validated phase successor. It neither converts material into `RuntimeSession` input nor treats local acknowledgement collection as the unfrozen portable acknowledgement-set fact, and it terminates before BCH signing admission. The internal runtime driver still accepts only `.opalV0`; mainnet-alpha rejects legacy aggregate host markers and has no public session, mailbox-to-runtime bridge, production host composition, durable recovery driver, transport route, or broadcast path. Commitment-opening/component linkage, authorization-response and complete acknowledgement-set schemas, previous-output resolution, anonymous BCH-signature authorization, concrete Tor integration, and independent review remain release blockers, so the new profile is not a live Mosaic or mainnet-readiness claim. `OpalFusion.Client.Session` remains the only live entry point and runs CashFusion.
