...
Status: the CashFusion engine is a pilot on `develop`; Mosaic remains non-live. Its deterministic foundations include the attempt and runtime reducers, an internal ordered host coordinator, the chipnet-only Opal v0 conformance contracts, and an additive mainnet-alpha contract profile that freezes role hashing, complete manifests, PlayerCommit, typed aggregate reassembly, authenticated control admission, pre-sign acknowledgements, BCH signature sets, and exact complete-transaction assembly. The coordinator proves in-process release, signing, exact-commit, cancellation, and recovery-required ordering against injected seams; mainnet-alpha remains deliberately runtime-disabled. Production commitment opening and lease-material construction, previous-output resolution, anonymous BCH-signature authorization, mailbox-to-runtime integration, concrete Tor networking, durable recovery, independent review, and all mainnet spending or broadcast remain unavailable. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
