...
The package now includes the protocol-neutral facade plus an internal peer-local Mosaic semantic core, deterministic multi-peer simulator, canonical primitive codec, strict NIP-01 event codec, NIP-44 encrypted-event boundary, authenticated replay guards, relay-publication quorum tracker, and narrow wallet-host contracts. Event kinds, tags, named payloads, transport adapters, and live effects remain unresolved or unimplemented, so these surfaces do not make Mosaic runnable over a network. `OpalFusion.Client.Session` remains the live CashFusion entry point; `OpalFusion.Session` intentionally has no public initializer yet.
