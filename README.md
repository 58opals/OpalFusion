...
Status: the CashFusion engine is a pilot on `develop`; Mosaic is a draft specification with a deterministic in-process semantic core, validated role-seed-to-roster integration, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and reassembly, strict NIP-01 event validation, an authenticated NIP-44 boundary, BIP340 manifest-round and contributor pre-sign acknowledgement validation, deterministic Opal v0 unsigned-transaction construction, locally derived transcript-root gating, and a fail-closed contributor-local inclusion-validation boundary, plus replay and relay-publication guards, wallet-host contracts, and an internal bounded RFC 9474 authorization path. Mosaic still has no production commitment-opening/inclusion validator, discovery, frozen Opal v0 role hash documents, complete manifest/BCH-signature/proof messages, previous-output-backed wallet validation, fragment-to-authenticated-envelope integration, live transport, independently reviewed cryptographic deployment, or live runtime claim. The remaining CashFusion proof gates are tracked in [CashFusion Native Swift Support Statement](docs/cashfusion-native-support-statement.md).
...
The package now includes the protocol-neutral facade plus an internal peer-local Mosaic semantic core, deterministic multi-peer simulator, validated role-seed-to-roster integration, canonical primitive and Opal v0 component-document codecs, bounded aggregate fragmentation and terminal reassembly, strict NIP-01 event codec, NIP-44 encrypted-event boundary, authenticated replay and strict Opal-v0 sequence guards, relay-publication quorum tracker, bounded RFC 9474 authorization execution, deterministic unsigned-transaction construction, locally derived transcript-root gating, a fail-closed contributor-local inclusion boundary, and generation-bound wallet-host contracts. The executable transcript binding accepts only `.opalV0`; `.draft1` fails closed. Opal v0 role-commitment and role-seed hash documents, a production commitment-opening/component-linkage validator, previous-output-backed validation, complete manifests, BCH-signature messages, proof/blame payloads, fragment-to-authenticated-envelope integration, transport adapters, and live effects remain unresolved or unimplemented, and the cryptographic path has not completed independent review, so these surfaces do not make Mosaic runnable over a network. `OpalFusion.Client.Session` remains the live CashFusion entry point; `OpalFusion.Session` intentionally has no public initializer yet.
