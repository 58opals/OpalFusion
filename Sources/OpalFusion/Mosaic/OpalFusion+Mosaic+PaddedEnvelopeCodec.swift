// OpalFusion+Mosaic+PaddedEnvelopeCodec.swift

extension OpalFusion.Mosaic {
    /// The fixed 8,192-byte hexadecimal envelope used by the frozen Mosaic transports.
    enum PaddedEnvelopeCodec: Sendable {
        ...
        static let maximumPayloadByteCount = 4_092
        static let encodedByteCount = 8_192
        ...
        static func encode(_ payload: [UInt8]) throws -> String {
            ...
            bytes.append(
                contentsOf: repeatElement(
                    UInt8(ascii: "0"),
                    count: encodedByteCount - bytes.count
                )
            )
        ...
        static func decode(_ encoded: String) throws -> [UInt8] {
            ...
            guard payloadLength <= maximumPayloadByteCount else {
        ...
        private static func lowercaseHexadecimal(
            _ value: UInt32,
            width: Int
        ) -> [UInt8] {
