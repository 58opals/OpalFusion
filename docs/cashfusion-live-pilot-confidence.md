...
This guide describes the opt-in Electron Cash live smoke gate for the current OpalFusion pilot path. Normal local and CI validation do not run this smoke because it requires a real Electron Cash `4.4.3`-compatible coordinator and wallet material supplied through the environment.
...
Run the current pilot-confidence gate of three consecutive successful smokes:
...
The live pilot-confidence runner proves that the configured environment can reach session-level success against a real coordinator. It does not produce pinned transcript fixtures and does not replace local replay evidence.
