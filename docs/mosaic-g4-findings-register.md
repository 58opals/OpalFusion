...
| `G4-GAP-003` | Parser fuzz | Historical graph locally verified: 92 OpalFusion seeds across 24 bodies, 52/0/0 low-level map, nine mapped composite parents with passing-evidence parity, and ten additional Wallet/OpalBase seeds across four bodies with consumed OpalCrypto roots mapped transitively; the later atomic-publication and canonical-journal delta is not byte-identical | Preserve both freshness maps and affected selectors; independent reviewers must disposition reuse across the successor delta and assess whether the deterministic first-party campaign is sufficient for the bounded private-alpha gate |
| `G4-GAP-004` | Simulator fault matrix | Locally closed for OpalFusion `3fd91a603b03bc59eecf0f3be6596fef8cfafde7` and Wallet `1370b7ddec32a047c2d2c3c2d5885a122fa40736`: every previously held runtime and overflow selector passed, alongside the changed publication, relay, recovery, and exact application-lifecycle suites | Bind the successor Wallet assurance manifest and exact-graph package CI result; independent reviewers must assess the changed fault coverage and preserved no-retry record |
...
| 2026-08-23 | Pinned two public recovery events and restored seven positive recovery discriminants across three focused parser bodies | `7003e40a695912a0e8d0bf15ca0a2c925327c9fc` | The 52-file OpalFusion map reaches 52 covered with zero partial or closure-budget dispositions; composite, cross-repository, and every-parser closure remain open |
| 2026-08-23 | Added a machine-checked map for all nine public transport composite byte parents and bound their unchanged passing G2 integration evidence | Git revision containing this update | Combined with Wallet's cross-repository map, every parser root in the frozen first-party graph is locally accounted for; independent sufficiency review and all other G4 gaps remain open |
| 2026-08-31 | Rebound the review target to atomic publication-journal, route-preflight, bounded-buffer, and exact Wallet persistence hardening; recorded that all previously held local runtime and overflow selectors now pass | Git revision containing this update | Local fault hold removed for review intake only; exact package CI, signed runtime, external environment, seven independent dispositions, canary, and residual-risk acceptance remain open |
