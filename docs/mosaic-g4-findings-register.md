...
| `G4-GAP-003` | Parser fuzz | Partial: first-party deterministic OpalFusion campaign covers 92 seeds across 24 focused parser test bodies; the 52-file map records 52 covered, zero partial recovery variants, and zero closure-budget files / composite and cross-repository reconciliation remain open | Complete the composite-parent and cross-repository root map with coverage evidence for every parser before live profile enablement |
