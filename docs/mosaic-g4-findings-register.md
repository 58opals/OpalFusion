...
| `G4-GAP-003` | Parser fuzz | Partial: first-party deterministic campaign covers 72 seeds across eleven focused test bodies and freezes a 52-file implementation registry / complete root reconciliation and every-parser coverage remain open | Fuzz target, seed corpus, reviewed root-to-target registry, and coverage evidence for every parser before live profile enablement; the current deterministic seeds may feed that lane but do not waive it |
...
| 2026-08-23 | Covered the manifest-proposal-candidate root and a validated-manifest, initialized-journal recovery snapshot, reaching 72 seeds across eleven focused test bodies | Git revision containing this update | The generic large-snapshot mutation loop was stopped over budget and replaced by six focused discriminant mutations; remaining root reconciliation and every independent disposition stay open |
