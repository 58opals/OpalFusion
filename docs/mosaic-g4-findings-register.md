...
| `G4-GAP-003` | Parser fuzz | Partial: first-party deterministic campaign covers 75 seeds across twelve focused test bodies and freezes a 52-file implementation registry / complete root reconciliation and every-parser coverage remain open | Fuzz target, seed corpus, reviewed root-to-target registry, and coverage evidence for every parser before live profile enablement; the current deterministic seeds may feed that lane but do not waive it |
