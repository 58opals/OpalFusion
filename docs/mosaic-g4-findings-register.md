...
| `G4-GAP-003` | Parser fuzz | Partial: first-party deterministic campaign covers 54 seeds across five focused test bodies / reviewed parser-root registry and every-parser reconciliation remain open | Fuzz target, seed corpus, root-to-target registry, and coverage evidence for every parser before live profile enablement; the current deterministic seeds may feed that lane but do not waive it |
