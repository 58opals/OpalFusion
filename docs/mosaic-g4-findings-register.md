...
| `G4-GAP-003` | Parser fuzz | Partial: first-party deterministic campaign covers 75 seeds across twelve focused test bodies; the 52-file map records 41 covered, three partial recovery variants, and eight closure-budget bootstrap files / cross-repository reconciliation and every-parser coverage remain open | Complete the named recovery variants, bootstrap closure lane, composite parents, and cross-repository root map with coverage evidence for every parser before live profile enablement |
