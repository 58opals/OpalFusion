...
This profile freezes an additive mainnet-targeted canonical contract without enabling a runnable public Mosaic session, live transport, broadcast, or production-support claim. An internal local-material builder derives exactly 23 component slots from one validated reservation lease, constructs and locally opens the grouped commitments, and binds two purpose-separated blind authorizations to every slot. The admission ledger seals admitted documents to one attempt, generation, material identifier, and round-scoped per-sender sequence epoch; a local conductor additionally admits component mailbox sequence zero and BCH-signature mailbox sequence one under distinct attempt keys, then emits a canonical input-indexed signature set only after every transaction input is present. The paired internal runtime consumes exact acknowledgement, signature-set, complete-payload, and previous-output-backed validation facts. An internal contributor executor composes the material builder, publication seams, previous-output source, and refined wallet host through local signing and byte-exact commit while preserving pre-sign release and post-sign recovery ordering. A separate internal conductor executor performs attempt-scoped dual authorization issuance, authenticated-loopback-gated aggregate publication, anonymous component and signature collection, previous-output-backed signature validation, and exact transaction assembly without wallet or broadcast authority. An internal post-manifest role selector constructs the paired runtime and forwards a serialized stream of already-authenticated deliveries and source closure. The generic transport-facing runtime driver still rejects this profile. Production stateful lease provenance, material persistence and erasure, authoritative app composition, authenticated mailbox transport, durable crash recovery, Tor, explicit broadcast approval, and independent deployment review remain gated in Section 12. No test or package default may spend or broadcast mainnet funds.
...
The profile-scoped conductor executor starts only from a fresh conductor runtime at manifest agreement. It owns two purpose-separated authorization-issuance ledgers, publishes response sets in canonical contributor order, waits for its own typed publications to return through authenticated control admission, accumulates only admitted anonymous components and BCH signatures, resolves previous outputs once for the admitted transcript, and assembles the exact complete transaction before candidate-bound validation. Its input seams carry already-authenticated typed values, while its publication seam carries locally constructed typed documents awaiting later authenticated admission; neither seam assigns Nostr kinds, tags, timing, relay policy, Tor routing, or a public session. The internal `PostManifestRuntimeDriver` constructs one fresh paired runtime, selects this executor or the contributor executor from the validated roster, rejects dependency-role mismatch and contributor anonymous ingress, and forwards source termination behind previously accepted deliveries. It owns no transport task, private key, wallet authority, durable recovery, or broadcast policy. Any failure or cancellation is terminal and retry requires a fresh runtime and fresh evaluator material.
...
- production mailbox ingress that authenticates the per-component sequence-zero and sequence-one route, delivers admitted ledger facts into the paired runtime session, and preserves terminal and cancellation ordering without exposing contributor identities;
