...
The alpha.4 contract, local-material builder, component and signature admission, paired runtime, and internal contributor and conductor coordinators are implemented while the mainnet driver remains disabled. The conductor proof is post-manifest, typed, injected, non-wallet, and nonbroadcast. Executable production runtime work still requires the documented stateful material ownership, authoritative app composition, authenticated mailbox/Tor transport, durable recovery, and independent review.
