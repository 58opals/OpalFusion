...
G0, G1, and G2 are complete at one exact first-party application graph: Wallet `ca15e50e0db858e64298a680dcdb51438edfbf38`, OpalBase `54ad1d1606226a6afa63ce89cb9ff02fcc46f4e3`, OpalFusion `117949b08f354f5a58b78075dbf14b2ec4e4f88f`, OpalCrypto `23425db04075a48405d65cf6a3e2254911e626eb`, SwiftFulcrum `24b44bb2458822d14121dfbd57321fda7ae539ea`, and OpalDiagnostics `7cd2e383309821e01903077c1e534174f9c8a964`. [Mosaic G1 Closure Evidence — 2026-08-21](mosaic-g1-closure-evidence-2026-08-21.md) records the durable authenticated recovery substrate, and [Mosaic G2 Closure Evidence — 2026-08-21](mosaic-g2-closure-evidence-2026-08-21.md) records the package producer, sole OpalBase facade, Wallet-owned encrypted transport material and policy, concrete Tor-only adapter, exact dependency parity, and local production-adapter loopback.

This is not a live BCH mainnet engine. The complete G2 transport substrate remains disabled and is not bootstrapped into an application session. The packages expose only a macOS private-alpha SPI and injected transport boundaries; no public Mosaic session, G3 application-session composition, external relay or Tor deployment, chain connection, broadcast, value movement, or release promotion is enabled.
...
Status: `in progress` as of 2026-08-21. OpalFusion `d4866fd39c0d0dbe7b8df803e4eb198e487e8b89` adds the smallest producer-owned terminal-provenance router so a future Base/application facade can consume one opaque termination without inspecting hidden Fusion state or manufacturing protocol evidence. Received completion, received abort, persisted local timeout, locally generated completion, and fail-closed transport loss are covered through the sole runtime owner. Package build, `mosaic-fast` 6/6 in 12.243 seconds, non-`@testable` consumer recovery 4/4 in 0.001 seconds, the amended local-completion case in 214.089 seconds, and the three existing execution/recovery cases 3/3 in 901.435 seconds passed. OpalBase application-session composition, previous-output authority retention, Wallet session supervision, synchronous companion-journal transaction ownership, exact host-commit rehearsal, route-loss disposition, and app-restart recovery remain open, so no G3 closure is claimed.
