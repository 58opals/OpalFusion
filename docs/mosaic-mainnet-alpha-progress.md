...
Status: `in progress` as of 2026-08-23. G3 closure makes the integrated private session available for assurance. The first application blocker is the known Wallet Release/preview boundary; deterministic repository-owned validation, vectors, fuzzing, simulator faults, multi-device and traffic-analysis evidence, privacy-safe observability, runbooks, review packets, and independent dispositions remain open.
