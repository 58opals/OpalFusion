...
G0 and G1 are complete at one exact first-party application graph. The package-owned recovery boundary remains the one recorded in the [2026-08-19 package-layer completion checkpoint](mosaic-package-layer-checkpoint-2026-08-19.md). OpalCrypto `8db85b6853c360105f2baf37fc83be48a0ef92da` owns the G1 authenticated-encryption and key-derivation calculations; OpalBase `3eeeb86d41a609f081c1e32e31d0d21ac6ff52e7` routes its Mosaic journal through those capabilities; and Wallet `4eb09a88742d3aeb9efa4a56d031ad15458c87ce` supplies the production macOS authenticated outer record, Keychain keys and independent anchor, synchronized atomic file replacement, bidirectional startup catalog, cross-process scope lock, exact inventory and tombstones, and package-authorized terminal erasure. [Mosaic G1 Closure Evidence — 2026-08-21](mosaic-g1-closure-evidence-2026-08-21.md) records the exact graph, producer checks, 38 focused Wallet tests, and the standalone 15-scenario fresh-process matrix. OpalFusion `e07151af7a4f5208153ed37b9533b3de7d8e0628`, pinned to OpalCrypto `23425db04075a48405d65cf6a3e2254911e626eb`, now supplies the G2 package-owned blind-authorized mailbox-distribution, complete-consensus, publication-recovery, and exact-three-source inbox boundary recorded in the [2026-08-21 G2 package evidence](mosaic-g2-package-evidence-2026-08-21.md).

This is not a live BCH mainnet engine. The G1 persistence backend and G2 package producer are compiled but not bootstrapped into an application session. The packages expose only a macOS private-alpha SPI and injected transport boundaries; no public Mosaic session, application-owned G2 recipient or mailbox lifecycle, concrete Tor or relay deployment, G3 application-session composition, external-network broadcast, or release promotion is enabled.
...
The dependency boundary remains acyclic and the exact profile pair fails closed. The package-owned recovery implementations and validation closeout are complete: Fusion restores formation, coordinator, admission/publication, reconnect, and protocol terminal evidence; Base restores wallet, guarded broadcast, chain, quarantine, and journal cleanup authority. Producer publication, downstream public pinning, exhaustive Fusion prefix restoration, the serialized SPI aggregate, full package lanes, and the missing-input ownership decision are complete. OpalFusion now also completes the package-owned G2 bootstrap contract through application-injected secret, persistence, and route capabilities. Wallet owns G1 durable storage, authenticated inventory/tombstones, rollback/deletion detection, cross-process exclusion, Keychain lifecycle, state enumeration, outer atomic composition, and physical deletion; it must extend that same authority for G2. Concrete Tor provisioning, application recipient and mailbox custody, G2 durability, application session supervision, controls, and live finality policy remain application work rather than hidden package substitutes.
...
Status: `in progress` as of 2026-08-21. OpalFusion implements package-owned signed discovery and pre-manifest execution plus bootstrap.1 blind-authorized public mailbox distribution, complete roster consensus, byte-identical per-relay publication recovery, replay-restored exact-three-source inboxes, stable concrete-actor route identity, and cancellation-safe cleanup through injected capabilities. Private-deployment.1 semantics and the G1 production persistence substrate are complete. Application-owned recipient and mailbox key generation and custody, G2 schema evolution and encrypted transport-material persistence, durable spent/acknowledgement/wrapper/replay inventories, authoritative endpoint and operator configuration, concrete Tor-only WebSockets with reviewed circuit isolation, and the under-30-second production-adapter loopback remain absent, so no G2 closure is claimed. Exact package revision and validation evidence are recorded in the [2026-08-21 G2 package evidence](mosaic-g2-package-evidence-2026-08-21.md).
