...
G0 and G1 are complete at one exact first-party application graph. The package-owned recovery boundary remains the one recorded in the [2026-08-19 package-layer completion checkpoint](mosaic-package-layer-checkpoint-2026-08-19.md). OpalCrypto `8db85b6853c360105f2baf37fc83be48a0ef92da` now also owns the authenticated-encryption and key-derivation calculations used above that boundary; OpalBase `3eeeb86d41a609f081c1e32e31d0d21ac6ff52e7` routes its Mosaic journal through those capabilities; and Wallet `4eb09a88742d3aeb9efa4a56d031ad15458c87ce` supplies the production macOS authenticated outer record, Keychain keys and independent anchor, synchronized atomic file replacement, bidirectional startup catalog, cross-process scope lock, exact inventory and tombstones, and package-authorized terminal erasure. [Mosaic G1 Closure Evidence — 2026-08-21](mosaic-g1-closure-evidence-2026-08-21.md) records the exact graph, producer checks, 38 focused Wallet tests, and the standalone 15-scenario fresh-process matrix.

This is not a live BCH mainnet engine. The G1 persistence backend is compiled but not bootstrapped into an application session. The packages expose only a macOS private-alpha SPI and injected transport boundaries; no public Mosaic session, G2 recipient or mailbox lifecycle, concrete Tor or relay deployment, G3 application-session composition, external-network broadcast, or release promotion is enabled.
...
The dependency boundary remains acyclic and the exact profile pair fails closed. The package-owned recovery implementations and validation closeout are complete: Fusion restores formation, coordinator, admission/publication, reconnect, and protocol terminal evidence; Base restores wallet, guarded broadcast, chain, quarantine, and journal cleanup authority. Producer publication, downstream public pinning, exhaustive Fusion prefix restoration, the serialized SPI aggregate, full package lanes, and the missing-input ownership decision are complete. There is no remaining package-layer closeout item in this scope. Wallet now owns durable storage, authenticated inventory/tombstones, rollback/deletion detection, cross-process exclusion, Keychain lifecycle, state enumeration, outer atomic composition, and physical deletion. Tor provisioning, recipient and mailbox lifecycle, application session supervision, controls, and live finality policy remain later-gate application work rather than package responsibilities.
