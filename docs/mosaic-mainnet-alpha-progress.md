...
Status: `in progress` as of 2026-08-23. G3 closure makes the integrated private session available for assurance. Wallet production source `6fcfca789087f77ec556b22feec37e7b7226ad1d` routes a closed aggregate lifecycle signal set through first-party OpalDiagnostics without a direct Wallet-to-OpalFusion dependency or secret-bearing fields. Wallet test-control revision `d0f232fb998730fad6282b536d51b6b4c5e210c9` produced the preserved 3/3 bounded result in 7.47 seconds; successor `11ae405a1d6cf894934f512ef1e5b981672b6c19` changed only the watchdog to enforce a 60-second wall-clock ceiling, so unchanged tests were not rerun. The changed production source passed one dependency-locked Release build in approximately 186 seconds. Wallet evidence revision `579584aee8ae8f1cc9794007954a5c5182a94609` retains the privacy, recovery-disable, and no-user-surface evidence and adds a manual-only five-package `xcode-27` matrix that archives the frozen commits, binds only ephemeral locks, keeps SwiftPM sandboxing enabled, and blocks resolved-checkout mismatch. Wallet manifest revision `8375558e6c540fc9b2e7534ee9818cfa2986aa48` marks observability and the no-user-surface UX/accessibility boundary locally verified, exact-graph CI definition ready but unexecuted, and the undrilled runbook partial. OpalFusion private test revision `f2ecfe8638434ad2b5e8c941edef624ce9132532` expands the first-party deterministic mutation campaign to 70 parser seeds across ten focused test bodies. All ten bodies have passing affected-selector evidence, but no single final combined run or aggregate elapsed time is claimed; a machine-checked registry freezes 52 low-level parser implementation files; and `Sources`, `Package.swift`, and `Package.resolved` remain byte-identical to public runtime `79ba5f91449b2c0d5cd6ec73c38fafd58cae0b46`. [The bounded parser-mutation evidence](mosaic-g4-parser-mutation-evidence-2026-08-23.md) and [parser-root registry](mosaic-g4-parser-root-registry-2026-08-23.md) record the exact results, current accounting, gaps, and non-proofs. Exact-graph package CI execution, complete root reconciliation, complete portable vectors, fuzz coverage for every parser, complete simulator faults, multi-device and supported-environment traffic analysis, signed isolated runbook verification, reviewer assignments, independent UXC and safe-claim verification, and every other independent disposition remain open.

Successor private test revision `103c85918e84328600b876647c4c17f8951d0dc4` advances the campaign to 75 seeds across twelve focused bodies by adding the context-validating manifest-proposal-candidate root, a validated-manifest initialized-journal recovery seed, both NIP-59 composite open roots, and an anonymous publication-permit recovery record. The changed typed-Nostr body passed in 20.003 seconds, the composite-open body passed in 5.371 seconds, and the expanded durable publication body passed in 6.848 seconds. A generic large-snapshot mutation loop exceeded the ordinary budget and was stopped without an unchanged retry; its focused replacement passed one positive restoration plus six exact discriminant rejections in 58.999 seconds. Production source and dependency declarations remain byte-identical to public `develop`, and no combined rerun is claimed.

The private [G4 Independent Review Packet](mosaic-g4-independent-review-packet-2026-08-23.md) now freezes seven non-self-certifying lanes and the exact source and evidence map, while the [G4 Findings Register](mosaic-g4-findings-register.md) records every lane as unassigned and preloads the known evidence gaps without treating them as independent findings. Packet preparation is complete; reviewer assignment, review work, remediation, retest, and every independent disposition remain open.
