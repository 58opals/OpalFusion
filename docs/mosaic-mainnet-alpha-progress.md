...
Status date: 2026-08-17. This is a non-normative implementation record; the protocol profile and security model remain authoritative.

## Current Claim

The package layer now preserves one authoritative Fusion runtime owner and one authoritative Base wallet-recovery actor across fresh and restored private-alpha attempts. OpalFusion source `251a3f0`, published with its checkpoint at `12c4575`, implements signed pre-manifest formation, bounded canonical recovery, exact admission/publication replay, contributor/conductor reconstruction, write-ahead protocol terminal records, deterministic reconnect barriers, and linear terminal evidence. OpalBase `0483fdc` includes the `140a951` recovery implementation, documentation, and exact public pins; it implements direct move-only Fusion/Base co-claim, journal v2 binding, deterministic wallet crash recovery, guarded broadcast presence reconciliation, chain/reorganization/finality handling, exact-owner quarantine release, and terminal journal-erasure authority. OpalCrypto `cc51fc4` supplies the exact RSA private-key restoration primitive. Crypto `cc51fc4` and Fusion `12c4575` are fetchable on public `develop`, Base private `draft` pins those exact revisions, and the clean public-URL Base lane passes. The exhaustive formation-prefix test and combined private-alpha SPI suite remain incomplete as recorded in the [2026-08-17 package-layer wrap-up checkpoint](mosaic-package-layer-checkpoint-2026-08-17.md). No application composition is current evidence.
...
| 12 | Exact package-head validation | A clean public-URL lane resolved Base `0483fdc` to Fusion `12c4575` and Crypto `cc51fc4`, verified checkout and remote-head parity, built successfully, and passed 37 network-target tests with live flags disabled plus 1,034 local tests across 108 suites. Fusion's repository fast and rehearsal gates passed, and no local path or private URL was committed. | [2026-08-17 checkpoint](mosaic-package-layer-checkpoint-2026-08-17.md) |
...
The dependency boundary remains acyclic and the exact profile pair fails closed. The package-owned recovery implementations are present: Fusion restores formation, coordinator, admission/publication, reconnect, and protocol terminal evidence; Base restores wallet, guarded broadcast, chain, quarantine, and journal cleanup authority. Required producer publication and the downstream public-pin lane are complete; remaining package closeout is exhaustive Fusion validation and a documented decision or implementation for missing-input tombstones rather than a missing alternate runtime. Application-owned durable storage, rollback/deletion detection, cross-process ownership, Keychain lifecycle, state enumeration, outer atomic composition, Tor provisioning, controls, finality policy, physical deletion, and supervision remain deliberately deferred rather than moved into either package.

## Validation Lanes

The focused OpalFusion contributor lane is `./scripts/run-validation-loop.sh mosaic-fast`. On Fusion source `251a3f0`, it passed 6/6 tests in two suites in 12.841 seconds after a 15.27-second build. OpalBase recovery validation centers on `AccountMosaicPrivateAlphaRecoveryOwnerValidator`, `AccountMosaicPrivateAlphaRuntimeAdapterValidator`, the planner/journal suites, and the broadcast/host suites; its clean public-URL lane at `0483fdc` passed 37 network-target tests with live flags disabled plus 1,034 local tests across 108 suites against exact Fusion `12c4575` and Crypto `cc51fc4` pins. The exhaustive Fusion formation-prefix test remains separately incomplete because cumulative canonical re-authentication reached nonce allocation at 1,148.994 seconds before the user-requested checkpoint. Exact evidence and non-proofs are recorded in the [2026-08-17 package-layer wrap-up checkpoint](mosaic-package-layer-checkpoint-2026-08-17.md).
...
Status: `in progress` as of 2026-08-17. Fusion `12c45757f2ff5185b31ed14902e3304680576636`, Base `0483fdc4d043ca792d8ba5a07695136af1a3e6ce`, and Crypto `cc51fc436c95381dfce25fd3f04377d6c29a17f3` form the exact fetchable package graph without changing alpha.4 or alpha.5. Fusion and Crypto are published on public `develop`; Base private `draft` pins those exact revisions, and the clean public-URL lane passes. Explicit approval of the private-deployment.1 semantic decisions, the complete application dependency graph, and every release gate remains open; no G0 closure is claimed.
...
Status: `in progress` as of 2026-08-17. Fusion source `251a3f0`, published at `12c4575`, restores signed formation prefixes, the existing post-manifest coordinator, exact admission/publication journals, reconnect barriers, and protocol terminal evidence. Base `0483fdc` executes authenticated wallet recovery plans exactly once, reconciles guarded broadcast and chain presence, releases exact-owner quarantine, and authorizes nonempty terminal journal erasure against exact public pins. The public-only dependency proof passes, but these package capabilities still depend on injected application storage and supervision; exhaustive Fusion prefix validation remains incomplete, and missing-input tombstones remain fail-closed rather than implemented. No G1 closure is claimed.
