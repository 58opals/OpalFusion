...
Successor private test revision `b3f4e2329fa7a53d175da462eb0aa2776dd8087c` advances the campaign to 72 seeds across eleven focused bodies by adding the context-validating manifest-proposal-candidate root and a validated-manifest, initialized-journal recovery seed. The changed typed-Nostr body passed in 20.003 seconds. A generic large-snapshot mutation loop exceeded the ordinary budget and was stopped without an unchanged retry; its focused replacement passed one positive restoration plus six exact discriminant rejections in 58.999 seconds. Production source and dependency declarations remain byte-identical to public `develop`, and no combined rerun is claimed.
