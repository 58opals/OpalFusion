...
Here `bytes(value)` is the Mosaic canonical `u32(length) || value` encoding. A `MosaicTranscriptBinding` validates all digest lengths, recomputes the unsigned-transaction digest from the exact bytes, and rejects an acknowledged root that differs from the recomputed root. `MosaicTransactionSigningRequest` requires this binding and rejects substituted transaction bytes. The host binding also recognizes the separate [`Mosaic/0-opal-mainnet-alpha.3`](mosaic-mainnet-alpha-profile.md) contract; `.draft1` fails closed because its complete manifest and canonical hash documents are not frozen. Recognition of the mainnet-alpha binding does not make this chipnet profile mainnet-capable or enable that profile's runtime driver.
