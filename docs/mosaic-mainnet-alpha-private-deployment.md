...
Status: Frozen private-deployment supplement. Private-deployment identifier: `nostr-tor/0-opal-mainnet-alpha-private-deployment.1`.

This document closes only the previously unresolved private-deployment contract decisions needed to compose `Mosaic/0-opal-mainnet-alpha.4` with `nostr-tor/0-opal-mainnet-alpha.5` in one reviewed macOS alpha. It does not change any alpha.4 protocol byte or alpha.5 post-manifest transport byte. The [Mosaic Mainnet-Alpha Profile](mosaic-mainnet-alpha-profile.md) remains authoritative for those frozen identifiers, the [Mosaic Security Model](mosaic-security-model.md) remains authoritative for claims and threats, and this supplement is authoritative only for the separately versioned discovery, pre-manifest, deployment-policy, and cross-layer recovery decisions below. A change to any field, domain, event kind, timing value, signer rule, endpoint rule, or canonical order in this supplement requires a new private-deployment identifier.
