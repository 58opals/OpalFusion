...
Status: Accepted frozen private-deployment supplement for the internal macOS private alpha as of 2026-08-21. Private-deployment identifier: `nostr-tor/0-opal-mainnet-alpha-private-deployment.1`. The semantic adoption record is [Mosaic Private-Deployment.1 Semantic Decision — 2026-08-21](mosaic-private-deployment-decision-2026-08-21.md).

This document freezes the previously unresolved private-deployment contract decisions needed to compose `Mosaic/0-opal-mainnet-alpha.4` with `nostr-tor/0-opal-mainnet-alpha.5` in one reviewed macOS alpha. The accepted values remain the implemented bytes and do not change any alpha.4 protocol byte or alpha.5 post-manifest transport byte. The [Mosaic Mainnet-Alpha Profile](mosaic-mainnet-alpha-profile.md) remains authoritative for those frozen identifiers, the [Mosaic Security Model](mosaic-security-model.md) remains authoritative for claims and threats, and this supplement is authoritative only for the separately versioned discovery, pre-manifest, deployment-policy, and cross-layer recovery contract below. A change to any field, domain, event kind, timing value, signer rule, endpoint rule, or canonical order in this supplement requires a new private-deployment identifier and separate approval.
...
After one complete manifest is authenticated, the separately versioned [`nostr-tor/0-opal-mosaic-private-alpha-bootstrap.1`](mosaic-private-alpha-transport-bootstrap.md) contract authenticates an attempt-exclusive blind-authorization key, complete control-mailbox claims, anonymous registrations, conductor-local public-key assignments, one unlabeled mailbox-bundle commitment set, and complete roster acknowledgements before the existing alpha.5 runtime capability can be minted. Bootstrap.1 is a post-manifest application handoff, not another private-deployment event kind, and it changes no canonical byte or deadline in this supplement.
...
OpalFusion owns the canonical documents, phase semantics, private mapping, authenticated bootstrap distribution contract, runtime restoration semantics, and transport behavior. OpalBase owns wallet selection, reservations, signing, recovery execution, previous-output validation, approval, broadcast, and chain reconciliation. SwiftFulcrum owns chain-client correctness. OpalCrypto owns primitives. The Wallet application owns key generation and custody, durable persistence, Keychain integration, Tor/relay provisioning, lifecycle supervision, user controls, cross-process ownership, and policy. OpalDiagnostics may record only approved privacy-safe state and error categories.
