...
Status: refreshed for independent reviewer assignment at the 2026-08-31 hardening candidate; reviewers must pair this packet revision with the Wallet manifest revision that binds it. No review lane is complete and no G4 disposition is implied.
...
| OpalFusion runtime | `3fd91a603b03bc59eecf0f3be6596fef8cfafde7` | Frozen private-alpha protocol, transport, bootstrap, restoration, exact-host, terminal-recovery, atomic publication-journal, route-preflight, and bounded-buffer implementation consumed by OpalBase |
| OpalFusion G4 parser harness | `7003e40a695912a0e8d0bf15ca0a2c925327c9fc` | Historical first-party bounded deterministic parser campaign for 92 positive seeds across 24 focused test bodies plus digest-pinned transport and recovery fixtures and a machine-checked 52-file implementation registry; reviewers must assess reuse against the later `3fd91a6` production delta rather than assuming byte identity |
| OpalFusion G4 parser evidence | Git revision containing this packet update | Machine-checked maps cover all 52 low-level implementation files and all nine public transport composite byte parents; Wallet's application-owned map adds ten Wallet/OpalBase seeds and consumed OpalCrypto accounting, bringing the frozen graph to 102 positive seeds across 28 focused bodies |
| OpalBase | `b48ee57c32793f65fe4a0c6b3e46b445dbf5fb0f` | Sole Wallet-facing runtime facade, wallet reservation and transaction validation, journal recovery, guarded broadcast, chain-policy authority, and isolated Fulcrum-process evidence; production source is unchanged from the prior Wallet-consumed revision |
...
| Wallet production source | `1370b7ddec32a047c2d2c3c2d5885a122fa40736` | Sole application session owner, authenticated persistence, Keychain custody, atomic relay-route batches, exact authenticated journal replacement, bounded validation fingerprints, concrete disabled transport, recovery, and aggregate observability |
...
| Wallet local closure evidence | `2b7b8e6b1c01faae80ce7f2a41ff996d96448af2` | Frozen source identity, complete serialized Wallet result, changed package suites, locally closed runtime and overflow selectors, platform builds, dependency parity, exact non-proofs, and remaining gates |
| Wallet G4 manifest | Wallet Git revision that binds this packet's OpalFusion commit | Exact graph, stage states, budgets, run conditions, authorization boundaries, and immutable evidence pointers; reviewers must record that exact Wallet revision rather than infer it from this packet |
| OpalFusion hardened runtime source | `3fd91a603b03bc59eecf0f3be6596fef8cfafde7` | Runtime ancestor of this documentation-bearing packet revision; the packet commit, not this ancestor alone, is the reviewable private `draft` and public `develop` candidate, while public `main` and release tags remain untouched |
...
- [`mosaic-g4-parser-mutation-evidence-2026-08-23.md`](mosaic-g4-parser-mutation-evidence-2026-08-23.md) owns the exact private test revision, bounded parser-mutation result, production parity, remaining coverage gap, and non-proofs.
...
| G4 local slices | Release compile boundary, machine assurance manifest, exact Debug and Release entrypoints, aggregate diagnostics, 3/3 bounded signal and disable proof, prepared no-network runbook, locally verified no-user-surface UX/accessibility boundary, a manual-only exact-graph package matrix with locally checked ephemeral locks, 62 passing historical golden-vector tests, locally verified first-party parser accounting for 102 positive seeds across 28 focused bodies with 52/0/0 low-level and nine-parent composite maps, and a 2026-08-31 hardening candidate whose previously held runtime and overflow selectors now pass | Exact package CI execution on the successor graph, independent sufficiency review of the parser campaign and hardening delta, multi-device and supported-environment evidence, signed runbook drill, future rendered-interface validation if one is introduced, and independent dispositions |
