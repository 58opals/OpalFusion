...
| OpalFusion G4 test-only harness | `5e237bfcb1d8402ba8f1f86b4d8260b18fa23f49` | First-party bounded deterministic mutation campaign for 85 parser seeds across 21 focused test bodies plus a digest-pinned transport fixture and machine-checked 52-file implementation registry; production source, manifest, and lockfile remain byte-identical to the runtime row |
| OpalFusion G4 parser evidence | Git revision containing this packet update | Machine-checked one-to-one root map for all 52 implementation files: 49 covered, three partial recovery variants, and zero closure-budget files; composite, cross-repository, and every-parser closure remain open |
