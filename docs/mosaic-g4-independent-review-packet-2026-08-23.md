...
| G4 local slices | Release compile boundary, machine assurance manifest, exact Debug and Release entrypoints, aggregate diagnostics, 3/3 bounded signal and disable proof, prepared no-network runbook, locally verified no-user-surface UX/accessibility boundary, a manual-only exact-graph package matrix with locally checked ephemeral locks, 62 passing locally verified golden-vector tests, 106 passing focused fault tests with an explicit OpalFusion timed hold, and locally verified first-party parser accounting for 102 positive seeds across 28 focused bodies with 52/0/0 low-level and nine-parent composite maps | Exact package CI execution, independent sufficiency review of the parser campaign, held OpalFusion fault selectors, multi-device and supported-environment evidence, signed runbook drill, future rendered-interface validation if one is introduced, and independent dispositions |
