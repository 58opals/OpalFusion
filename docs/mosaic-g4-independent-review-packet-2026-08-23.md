...
| OpalFusion G4 test-only harness | `7003e40a695912a0e8d0bf15ca0a2c925327c9fc` | First-party bounded deterministic parser campaign for 92 positive seeds across 24 focused test bodies plus digest-pinned transport and recovery fixtures and a machine-checked 52-file implementation registry; production source, manifest, and lockfile remain byte-identical to the runtime row |
| OpalFusion G4 parser evidence | Git revision containing this packet update | Machine-checked one-to-one root map for all 52 implementation files: 52 covered, zero partial recovery variants, and zero closure-budget files; composite, cross-repository, and every-parser closure remain open |
...
| Wallet G4 evidence | `e894f921b048a84587a616ccdf57dc81a73fa707` | Privacy contract, bounded results, recovery-disable runbook, no-user-surface proof, prepared manual-only exact-graph package matrix, locally verified golden vectors, and consolidated partial fault evidence |
| Wallet G4 manifest | `27efcff54916d8955895289fed4980fbcd0cf693` | Exact graph, stage states, budgets, run conditions, authorization boundaries, and immutable evidence pointers |
...
| G4 local slices | Release compile boundary, machine assurance manifest, exact Debug and Release entrypoints, aggregate diagnostics, 3/3 bounded signal and disable proof, prepared no-network runbook, locally verified no-user-surface UX/accessibility boundary, a manual-only exact-graph package matrix with locally checked ephemeral locks, 62 passing locally verified golden-vector tests, 106 passing focused fault tests with an explicit OpalFusion timed hold, passing affected-selector evidence for a first-party parser campaign covering 92 positive seeds across 24 test bodies, and a machine-checked 52-covered / 0-partial-variant / 0-closure-budget parser root map | Exact package CI execution, complete composite and cross-repository root reconciliation, fuzz coverage for every parser, held OpalFusion fault selectors, multi-device and supported-environment evidence, signed runbook drill, future rendered-interface validation if one is introduced, and independent dispositions |
