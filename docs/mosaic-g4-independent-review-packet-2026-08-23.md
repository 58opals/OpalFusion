...
| OpalFusion G4 test-only harness | `8ba234e7a841cee6e5ccbf14dd37068f53dcd6e5` | First-party bounded deterministic mutation campaign for an initial 20 parser seeds; production source, manifest, and lockfile remain byte-identical to the runtime row |
...
| G4 local slices | Release compile boundary, machine assurance manifest, exact Debug and Release entrypoints, aggregate diagnostics, 3/3 bounded signal and disable proof, prepared no-network runbook, locally verified no-user-surface UX/accessibility boundary, a manual-only exact-graph package matrix with locally checked ephemeral locks, and a 3/3 first-party deterministic mutation campaign for 20 initial parser seeds | Exact package CI execution, complete portable vectors, fuzz coverage for every parser, complete simulator faults, multi-device and supported-environment evidence, signed runbook drill, future rendered-interface validation if one is introduced, and independent dispositions |
