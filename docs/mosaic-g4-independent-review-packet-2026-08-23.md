...
| G4 local slices | Release compile boundary, machine assurance manifest, exact Debug and Release entrypoints, aggregate diagnostics, 3/3 bounded signal and disable proof, prepared no-network runbook, locally verified no-user-surface UX/accessibility boundary, a manual-only exact-graph package matrix with locally checked ephemeral locks, passing affected-selector evidence for a first-party deterministic mutation campaign covering 75 parser seeds across twelve test bodies, and a machine-checked 52-file parser implementation registry | Exact package CI execution, complete root reconciliation, complete portable vectors, fuzz coverage for every parser, complete simulator faults, multi-device and supported-environment evidence, signed runbook drill, future rendered-interface validation if one is introduced, and independent dispositions |
