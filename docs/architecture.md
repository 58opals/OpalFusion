...
| Inbound fan-in | Relay fan-in starts and drains provisioned mailbox groups; ingress authenticates the recipient capability; the admission journal records accepted replay facts; the runtime driver routes the typed delivery | Fan-in preserves source order and source-loss behavior. The caller supplies durable storage; nonempty restart state requires full runtime recovery and therefore fails closed. |
