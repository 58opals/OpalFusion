...
- `OpalFusion.Mosaic.Configuration` selects one authoritative `.draft1` or `.opalV0` profile and derives its protocol, roster, and transport contracts. Internal aggregate and peer-local reducers enforce the bounded phase, roster, reservation, transcript, generation, and terminal rules. Canonical Section 8 primitives, strict NIP-01 signed-event validation, an outer-authenticated NIP-44 encrypted-event boundary, replay guards, a three-relay/two-acceptance publication tracker, wallet-host contracts, and the deterministic Opal v0 issuance, bounded RFC 9474 execution, transcript, padding, tag, kind, and strict-sequence contracts are implemented. Discovery wire messages, live transport, effect driving, independent cryptographic review, and a live round runtime are not.
...
The Mosaic attempt reducers consume explicitly validated aggregate facts and emit peer-local effects for reservation eligibility, signing eligibility, reservation release or commit, and terminal outcomes. The package also validates transcript-bound host requests, canonical primitive encodings, NIP-01 event identifiers and signatures, NIP-44 encrypted content after outer-event authentication, generic draft replay behavior, Opal v0 strict sequences, one-time authorization reuse and issuance accounting, bounded blind-authorization execution, and relay acceptance quorum. It does not define complete named wire-message layouts, provide an independently reviewed production cryptographic deployment, execute wallet or network effects, or make the public `OpalFusion.Session` runnable.
