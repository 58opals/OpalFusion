...
This guide maps the current package and its intended two-engine architecture. The normative hierarchy is [Opal Fusion Specification](opal-fusion-specification.md); protocol behavior is owned separately by [CashFusion Implementation Spec](cashfusion-implementation-spec.md) and [Mosaic Protocol Specification](mosaic-protocol-specification.md). The frozen non-live mainnet contract and separately versioned deployment decisions are owned by the [Mosaic Mainnet-Alpha Profile](mosaic-mainnet-alpha-profile.md) and [Mosaic Mainnet-Alpha Private-Deployment Profile](mosaic-mainnet-alpha-private-deployment.md). Current package capabilities and limitations are summarized in the [README](../README.md); application obligations and release requirements remain in the profiles and security model.
...
`AdmissionLedger` alone stores the raw `PreSignAcknowledgementSet` and exports operation-specific transcript-acknowledgement validations. The reservation coordinator retains those validated artifacts; runtime rechecks profile, round, and transcript root, and `SigningRequestBuilder` also checks exact contributor order. Control and anonymous publication share concrete route-allocation and closure mechanics while retaining separate purpose, timing-permit, recipient, and batch policies. Cryptographic computation remains owned by OpalCrypto, and wallet reservation, signing, persistence, approval, and broadcast remain owned by OpalBase and the integrating application.
