...
| Admission and replay | `AdmissionLedger` for attempt-local semantic admission and sender sequence use | Durable digest merging, sender-sequence restoration, and crash replay are not yet implemented. |
...
| Outbound publication | Control/anonymous bridges seal exact context; batch publishers bind recipient route groups; relay publishers own one EVENT/ACK operation | None of these layers owns semantic loopback, retry persistence, endpoint selection, or Tor provisioning. |
| Inbound fan-in | Relay fan-in starts and drains provisioned mailbox groups; ingress authenticates the recipient capability; the runtime driver routes the typed delivery | Fan-in preserves accepted copies and source order but does not yet durably merge relay duplicates or restore replay state. |
...
The internal contributor transport bridge is one attempt-scoped binding above those publishers. It derives the control context from the validated bootstrap and manifest, matches the contributor's inbound control capability to the exact roster recipient key, mints a route group with a one-shot attempt claim, installs the anonymous path only after the exact lease-backed local material returns, preserves the coordinator's PlayerCommit → components → acknowledgement → signatures publication order, and separates a reentrancy-safe stop request from the lifecycle owner's termination wait. It owns no wallet operation, fan-in startup or supervision, automatic source-loss wiring, timing policy, route allocation, semantic loopback, persistence, retry, or public session.
