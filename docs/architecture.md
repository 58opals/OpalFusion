...
- `OpalFusion.Mosaic.Configuration` selects one authoritative `.draft1` or `.opalV0` profile and derives its protocol, roster, and transport contracts. Internal aggregate and peer-local reducers enforce the bounded phase, roster, reservation, transcript, generation, and terminal rules. The reducer also enforces the complete role-commitment barrier and derives the roster itself from a profile-bound validated 32-byte role seed using explicit big-endian selection; no caller supplies roles or a conductor. Canonical Section 8 primitives, Opal v0 commitment/component/set and bounded message documents, bounded aggregate fragmentation and terminal reassembly, strict NIP-01 signed-event validation, an outer-authenticated NIP-44 encrypted-event boundary, BIP340 manifest-round and contributor pre-sign acknowledgement validation, deterministic Opal v0 unsigned-transaction construction with exact ordering and aggregate-fee validation, locally derived transcript-root gating, a contributor-local exact-attempt/material/transcript inclusion gate, replay guards, a three-relay/two-acceptance publication tracker, wallet-host contracts, and the deterministic Opal v0 issuance, bounded RFC 9474 execution, transcript, padding, tag, kind, and strict-sequence contracts are implemented. Opal v0 role-commitment and role-seed hash documents, a production commitment-opening/inclusion validator, discovery wire messages, complete manifests and BCH-signature messages, previous-output-backed wallet validation, fragment-to-authenticated-envelope integration, proof/blame payloads, live transport, effect driving, independent cryptographic review, and a live round runtime are not.
