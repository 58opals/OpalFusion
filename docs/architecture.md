...
| Inbound fan-in | The attempt transport owner issues one role-complete inbound-runtime provisioning capability; relay fan-in consumes its single attempt binding, starts and drains the mailboxes, and ingress authenticates recipients before typed delivery | The capability authorizes only the exact private fan-in construction. It carries no admission, wallet, or broadcast authority; concrete Tor proof, durable storage, and full runtime recovery remain external. |
...
Private mainnet runtime construction has a separate one-shot gate. Only a successfully initialized attempt transport owner can mint the opaque inbound-runtime provisioning value, and fan-in rejects a foreign bootstrap, role substitution, or second claim before constructing ingress and the specialized driver. The generic `RuntimeSessionDriver` remains disabled for `.opalMainnetAlpha`, `OpalFusion.Session` remains privately initialized, and the capability cannot carry transaction bytes or reach a broadcaster. After an exact host commit, OpalBase separately derives a committed candidate and retains its existing security-profile, network-binding, persisted app-approval, persisted broadcast-intent, and network-client gates; none is satisfied by OpalFusion runtime authorization.
