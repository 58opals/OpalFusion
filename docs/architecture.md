...
- `OpalFusion.Mosaic.Configuration` selects one authoritative `.draft1` or `.opalV0` profile and derives its protocol, roster, and transport contracts. Internal aggregate and peer-local reducers enforce the bounded phase, roster, reservation, transcript, generation, and terminal rules. Canonical Section 8 primitives, Opal v0 commitment/component/set and bounded message documents, bounded aggregate fragmentation and terminal reassembly, strict NIP-01 signed-event validation, an outer-authenticated NIP-44 encrypted-event boundary, replay guards, a three-relay/two-acceptance publication tracker, wallet-host contracts, and the deterministic Opal v0 issuance, bounded RFC 9474 execution, transcript, padding, tag, kind, and strict-sequence contracts are implemented. Discovery wire messages, complete manifests and BCH-signature messages, fragment-to-authenticated-envelope integration, proof/blame payloads, live transport, effect driving, independent cryptographic review, and a live round runtime are not.
