...
- `OpalFusion.Mosaic.Configuration` selects one authoritative `.draft1`, `.opalV0`, or `.opalMainnetAlpha` profile. Opal v0 supplies the chipnet conformance and internal runtime-driver path. Mainnet-alpha.4 adds exact 23-slot local material, salt and Pedersen openings, dual purpose-separated authorization, typed control/anonymous admission, and canonical signature/transaction documents without adding a runnable session. `LocalContributionMaterial` is constructible only through its validated builder and can mint reservation-publication and transcript-inclusion facts for its exact attempt, generation, material, contributor, manifest, and lease; production live-lease ownership, persistence, freshness across retries, and erasure remain an external material-owner responsibility. `AdmissionLedger` separately admits component sequence zero and BCH-signature sequence one at a local conductor using one-time x-only sender and recipient-mailbox identities. The paired `OpalMainnetAlpha.RuntimeSession` still stops at transcript agreement, and `RuntimeSessionDriver` still rejects mainnet-alpha.
...
The Mosaic reducers consume validated aggregate publications, generation-bound host facts, and contributor-local validation tokens, while `RuntimeCoordinator` preserves reservation release, post-sign recovery, exact commit, and terminal ordering against injected seams. Alpha.4's admission ledger owns canonical control replay and standalone anonymous collection: it requires complete PlayerCommit and material-bound response-set evidence, rejects same-x communication-key reuse across the aggregate and anonymous paths, admits purpose-zero component tokens at mailbox sequence zero, admits purpose-one BCH-signature tokens at sequence one only after the exact acknowledgement set, and emits a sorted signature set only when every input is present. Authorization is an accounting and replay authority, not a grouped-commitment membership proof; the versioned profile explicitly accepts the off-commitment limitation. The previous-output resolver, signing-request builder, assembler, and public host contracts remain separate from the disabled mainnet runtime. Production mailbox/Tor ingress, stateful material ownership, previous-output-backed signature semantics in executable composition, host finalization/commit, durable recovery, and broadcast are not implemented.
