...
| Inbound fan-in | The attempt transport owner issues one role-complete inbound-runtime provisioning capability; fan-in consumes its single claim and passes one unforgeable immutable token through ingress to the specialized driver | Raw route data can be structurally validated and exercised against an inert endpoint without constructing a runtime. The capability carries no admission, wallet, or broadcast authority; concrete Tor proof, durable storage, and full runtime recovery remain external. |
