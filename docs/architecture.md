...
- `OpalFusion.Mosaic.Configuration` selects one authoritative `.draft1`, `.opalV0`, or `.opalMainnetAlpha` profile and derives its protocol, roster, and transport contracts. Internal aggregate and peer-local reducers enforce the bounded phase, roster, reservation, transcript, generation, and terminal rules and derive the conductor from a validated role seed. An internal ordered coordinator executes validated local host effects, drains late reservations to release before signing, converts ambiguous post-sign failures to recovery-required outcomes, and commits only through `MosaicCompleteTransactionHost`; its material builders and complete-transaction provider remain injected seams. Opal v0 supplies the chipnet conformance and internal runtime-driver path. Mainnet-alpha additively freezes the mainnet profile identity, deterministic fixed-overhead fee allocation, grouped Pedersen balance validation, role documents, complete manifest, PlayerCommit, per-contributor authorization-response sets, the complete contributor acknowledgement set, kind-bounded strict aggregate reassembly, authenticated control admission, BCH signature sets, exact 100-byte P2PKH unlocking scripts, and complete-transaction payloads. The public reservation and signing requests carry the exact roster-derived excess share, while `MosaicPreviousOutputSource` supplies ordered transcript input data without granting signing or broadcast authority. Their legacy initializers accept only a singleton range. Its sealed admission ledger binds admitted documents through transcript agreement to one attempt, generation, round, roster, local role, a roster-derived zero control-sequence epoch, and a monotonic externally validated phase mirror. A paired internal `OpalMainnetAlpha.RuntimeSession` advances the same validated local attempt only when ledger readiness and reducer state agree; contributor advancement requires an externally sealed reservation-publication validation carrying the exact lease and PlayerCommit binding, followed by exact local commitment inclusion in the conductor set. A narrow `OpalMainnetAlpha.ReservationCoordinator` compares that sealed validation with the actual host-returned lease, rejects direct validation injection and non-fresh runtime state, and drains cancellation to release without claiming that the lease produced the PlayerCommit. An internal `OpalMainnetAlpha.SigningRequestBuilder` requires the sealed lease publication, exact local transcript inclusion, a roster-complete acknowledgement set for the same root, and resolver-minted previous-output validation before mapping local inputs into a host signing request. That builder does not invoke the host, authorize signing, or enable the mainnet driver. Discovery, salt/opening and privacy-preserving component linkage, production lease-to-material construction and PlayerCommit derivation, a production previous-output source in executable runtime composition, anonymous BCH-signature authorization, production mailbox ingress, a concrete Tor connection, full production coordinator composition, durable recovery, independent review, and a public live Mosaic runtime are not implemented.
...
The Mosaic reducers consume validated aggregate publications, signatures, generation-bound host facts, and contributor-local validation tokens, then emit reservation, inclusion, acknowledgement, signing, disposition, and terminal effects. `RuntimeCoordinator` serially executes those local effects without cancelling in-flight wallet calls: source loss during reservation releases the late lease, cancellation after signing may have begun never releases it, and reducer completion becomes externally complete only after exact complete-transaction commit. Mainnet-alpha control admission seals implemented aggregate reservations and individual pre-sign documents to the authenticated sender, round, phase, roster, and publisher role, while typed reassembly rejects digest-valid malformed bytes. `AdmissionLedger` adds strict per-sender replay, independent fragment runs, exact manifest-proposal binding, complete conductor-local PlayerCommit and response-set collection, contributor-local blind-response finalization, one-time anonymous sender and mailbox identities, and exact complete acknowledgement-set publication. Its phase changes require attempt- and generation-bound validation from the paired mainnet-alpha runtime session, which carries an externally validated reservation-publication binding and verifies every local commitment is present before driving the local attempt through transcript agreement. The bridge deliberately withholds the portable acknowledgement set from BCH signing. Separately, a transcript-scoped resolver converts mainnet-alpha input components into an exact ordered `MosaicPreviousOutputSource` request and rejects source substitution. The signing-request builder accepts only the resolver-sealed result plus exact lease publication, local transcript inclusion, and complete acknowledgements, validates local outpoint, amount, P2PKH key, output, index, and fee-share bindings, and constructs the existing public host request without performing signing. The package can separately assemble and verify an exact complete P2PKH Schnorr transaction when supplied the acknowledged transcript and resolved previous outputs. It still cannot prove commitment openings, construct production lease material, validate live lease-to-PlayerCommit provenance, provide a production previous-output source in executable runtime composition, authorize anonymous BCH-signature mailbox submissions, persist or execute crash recovery, or select a live transport. Those missing authorities keep mainnet-alpha outside `RuntimeSessionDriver` and keep `OpalFusion.Session` non-runnable.
