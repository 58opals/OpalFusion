...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. Its fail-closed RSA seam, issuance accounting, canonical commitment/component documents, bounded aggregate fragmentation and reassembly, strict per-sender runtime sequencing, generation-bound host-result handling, deterministic unsigned-transaction construction, exact checks over component-declared aggregate values and fees, locally derived Opal-v0-only transcript binding, exact local attempt/material/transcript inclusion gate, envelope coding, and host contracts reduce ambiguity. The commitment-opening/component-linkage algorithm, production local-inclusion validator, and previous-output-backed value and ownership verification remain deferred, so these boundaries do not satisfy the remaining schema, transport, cryptographic-review, interoperability, or privacy release gates below.
