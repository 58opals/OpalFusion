...
| Sybil pool capture | The private deployment requires a 20-leading-zero-bit beacon throttle, oversubscription, and role randomness | These only increase cost; they do not provide strong Sybil resistance |
...
Mosaic uses proof of work and oversubscription only as throttles. The private-deployment.1 supplement fixes 20 leading zero bits after an Apple M1 Max measurement, but that device-specific result is neither an adversarial cost model nor a security level. A well-funded adversary may still control most or all selected identities, become conductor, or repeatedly abort attempts. No documentation or wallet UI may imply otherwise.
...
Nostr provides replicated signed events and convenient relay discovery; it does not provide network anonymity. NIP-44 explicitly notes metadata, forward-secrecy, post-compromise, and IP-observation limits. The separately versioned [private-deployment.1 supplement](mosaic-mainnet-alpha-private-deployment.md) fixes ephemeral discovery and pre-manifest kinds, exact signer authority, canonical relay selection, deadlines, abort, and completion without changing the post-manifest mapping. Those events expose discovery or control identities and timing to selected relays; they are coordination traffic, not anonymous traffic. The mainnet-alpha.5 adapter fixes an 8,192-byte padded kind-78 rumor, regular kind-1059 delivery, exact tags, cover-timestamp bounds, sender/recipient authority, and signed-wrapper replay identity. Fan-in preserves each route's order and forwards signed copies through one serialized ingress. After authentication, coordinators stage semantic admission and the injected attempt-bound journal records one accepted digest/sequence or wrapper/mailbox fact before any resulting effect; later relay copies append nothing. This establishes a durable replay boundary only when the caller supplies a durable store. It does not restore recipient keys, admission phase, coordinator work, wallet disposition, or publication state, so a nonempty journal fails closed on startup. Distinct injected connection objects prevent accidental reuse but do not prove independent Tor circuits. These adapters do not track remote wrapper-key freshness, generate or distribute recipient keys, reconnect, or prove concrete Tor routing. A gift wrap hides its authenticated sender from relay-visible content only when surrounding traffic does not relink it; the recipient mailbox key and traffic metadata remain visible.
