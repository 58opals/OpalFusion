...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.1`](mosaic-mainnet-alpha-profile.md) slice freezes deterministic role, manifest, PlayerCommit, aggregate, control-envelope, transcript, BCH-signature-set, and complete-transaction contracts, but remains runtime-disabled and rejects legacy aggregate host markers. Its admission-only ledger binds admitted control and anonymous-component documents through transcript agreement to one attempt, generation, round, roster, local role, explicit sequence baseline, and monotonic externally validated phase mirror; it cannot advance the runtime, treat locally collected individual acknowledgements as portable unanimity, or admit BCH signing and completion. An internal profile-neutral coordinator proves in-process host-effect ordering, including late-lease release, the no-release boundary after signing may have begun, exact complete-transaction commit, and recovery-required outcomes against injected seams. Neither profile proves the unfrozen authorization-response and complete acknowledgement-set contracts, a concrete Tor route, commitment-opening/component linkage, previous-output-backed wallet ownership and value validation, anonymous BCH-signature authorization, mailbox integration, production coordinator composition, durable recovery, or independent deployment review. These boundaries therefore do not satisfy the remaining transport, cryptographic-review, interoperability, mainnet-execution, or privacy release gates below.
