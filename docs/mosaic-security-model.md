...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.1`](mosaic-mainnet-alpha-profile.md) slice freezes deterministic role, manifest, PlayerCommit, aggregate, control-envelope, transcript, BCH-signature-set, and complete-transaction contracts, but remains runtime-disabled and rejects legacy aggregate host markers. Neither profile proves a concrete Tor route, commitment-opening/component linkage, previous-output-backed wallet ownership and value validation, anonymous BCH-signature authorization, mailbox integration, failure-aware effect execution and recovery, or independent deployment review. These boundaries therefore do not satisfy the remaining transport, cryptographic-review, interoperability, mainnet-execution, or privacy release gates below.
