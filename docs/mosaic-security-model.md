...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. Its fail-closed RSA seam, issuance accounting, canonical commitment/component documents, bounded aggregate fragmentation and reassembly, strict NIP-01 event and relay-frame validation, per-connection relay correlation, strict per-sender runtime sequencing, ordered post-admission runtime driving, generation-bound host-result handling, deterministic unsigned-transaction construction, exact checks over component-declared aggregate values and fees, locally derived Opal-v0-only transcript binding, exact local attempt/material/transcript inclusion gate, envelope coding, and host contracts reduce ambiguity. The relay session depends on an injected capability and does not prove a concrete Tor route, while the runtime driver does not execute wallet effects. The commitment-opening/component-linkage algorithm, production local-inclusion validator, previous-output-backed value and ownership verification, mailbox integration, failure-aware effect execution, and independent deployment review remain deferred, so these boundaries do not satisfy the remaining schema, transport, cryptographic-review, interoperability, mainnet, or privacy release gates below.
