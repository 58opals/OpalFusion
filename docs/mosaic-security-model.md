...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.3`](mosaic-mainnet-alpha-profile.md) slice freezes deterministic role, manifest, PlayerCommit, fixed-overhead fee allocation, grouped Pedersen balance, authorization-response-set, complete contributor acknowledgement-set, aggregate, control-envelope, transcript, BCH-signature-set, and complete-transaction contracts, but remains runtime-driver-disabled and rejects legacy aggregate host markers. Its admission ledger binds admitted control and anonymous-component documents through transcript agreement to one attempt, generation, round, roster, local role, roster-derived zero sequence epoch, and monotonic externally validated phase mirror. A paired internal runtime session advances the same validated local attempt through transcript agreement only after an external validator seals the exact lease and PlayerCommit binding and every local commitment appears in the conductor set; it cannot admit BCH signing or completion. A narrow reservation coordinator additionally compares the complete sealed lease with the actual host-returned lease, rejects non-fresh runtime state, and releases a late lease when cancellation wins. A public previous-output source contract returns ordered source-asserted output data, and an internal resolver binds transcript-ordered outpoints and amounts and rejects reported token presence without granting signing authority. An internal signing-request builder then requires that resolver-only validation, exact local transcript inclusion, and the roster-complete acknowledgement set before constructing a host request; it cannot invoke signing or bypass the disabled mainnet runtime. An internal profile-neutral coordinator separately proves in-process host-effect ordering, including the no-release boundary after signing may have begun, exact complete-transaction commit, and recovery-required outcomes against injected seams. Neither profile proves that the host lease produced the PlayerCommit, a concrete Tor route, salt/opening and privacy-preserving component linkage, production lease-material construction, authoritative previous-output-backed wallet ownership, script, token, and value validation in executable composition, anonymous BCH-signature authorization, production mailbox ingress, full coordinator composition, durable recovery, or independent deployment review. These boundaries therefore do not satisfy the remaining transport, cryptographic-review, interoperability, mainnet-execution, or privacy release gates below.
