...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.4`](mosaic-mainnet-alpha-profile.md) slice freezes exact 23-slot material, grouped balance, purpose-separated blind authorization, acknowledgements, authenticated anonymous admission, signature sets, and complete-transaction assembly. Its admission ledger binds accepted material to one attempt, generation, material, round, roster, phase, sender sequence, mailbox, authorization token, and input index; exact duplicates are idempotent and semantic rejection consumes no replay state. Alpha.4 deliberately does not prove that an authorized component opens a contributor-tagged grouped commitment, so a malicious contributor can spend an accounted token on another valid balanced component or cause an unattributable abort. Honest contributors still verify exact local inclusion, and OpalBase remains the wallet authority. The `nostr-tor/0-opal-mainnet-alpha.5` adapter authenticates regular NIP-59 gift wraps against one immutable recipient capability, seals complete control and anonymous publication batches, requires two accepted acknowledgements across three injected Tor-only routes, and starts three globally distinct subscriptions per mailbox while preserving source order. One attempt-bound admission journal sends only semantically accepted control digest/sequence and anonymous wrapper/mailbox replay facts through its injected append boundary before coordinator effects; identical relay copies append once, inputs that consume no replay state append nothing, and append failure discards staged runtime mutation. A separate outbound publication journal records complete batches, permits, relay attempts, acknowledgements, and terminal outcomes before their effects and reconciles exact readback after uncertain appends. Existing terminal rejection rules remain unchanged. A restored nonempty admission journal still fails closed because phase, coordinator, wallet, and inbound restoration remain unavailable; outbound continuation alone cannot construct a runtime. The adapters do not generate, distribute, or persist recipient keys, allocate authoritative routes, track wrapper-key freshness, reconnect, or prove concrete Tor isolation. The generic mainnet driver remains disabled and no public Mosaic session exists. Production durable storage, material ownership, authoritative app composition, full crash continuation, concrete Tor execution, independent review, and mainnet broadcast remain release gates; these boundaries are not an anonymity, accountability, or readiness claim.
...
Mosaic uses proof of work and oversubscription only as throttles. The proposed private-deployment.1 supplement selects 20 leading zero bits after an Apple M1 Max measurement, but that device-specific result is neither an adversarial cost model nor a security level, and adoption remains subject to explicit G0 semantic approval. A well-funded adversary may still control most or all selected identities, become conductor, or repeatedly abort attempts. No documentation or wallet UI may imply otherwise.
...
Nostr provides replicated signed events and convenient relay discovery; it does not provide network anonymity. NIP-44 explicitly notes metadata, forward-secrecy, post-compromise, and IP-observation limits. The separately versioned proposed [private-deployment.1 supplement](mosaic-mainnet-alpha-private-deployment.md) fixes candidate ephemeral discovery and pre-manifest kinds, exact signer authority, canonical relay selection, deadlines, abort, and completion without changing the post-manifest mapping; adoption remains pending the roadmap's explicit G0 semantic approval. Those events expose discovery or control identities and timing to selected relays; they are coordination traffic, not anonymous traffic. The mainnet-alpha.5 adapter fixes an 8,192-byte padded kind-78 rumor, regular kind-1059 delivery, exact tags, cover-timestamp bounds, sender/recipient authority, and signed-wrapper replay identity. Fan-in preserves each route's order and forwards signed copies through one serialized ingress. After authentication, coordinators stage semantic admission and the injected admission journal records one accepted digest/sequence or wrapper/mailbox fact before any resulting effect; later relay copies append nothing. A separate outbound publication journal records and can restore exact batch, permit, attempt, acknowledgement, and terminal state through an injected store. These establish durable component boundaries only when the caller supplies durable stores. They do not restore recipient keys, admission phase, coordinator work, wallet disposition, or inbound runtime, so a nonempty admission journal still fails closed on startup. Distinct injected connection objects prevent accidental reuse but do not prove independent Tor circuits. These adapters do not track remote wrapper-key freshness, generate or distribute recipient keys, reconnect, or prove concrete Tor routing. A gift wrap hides its authenticated sender from relay-visible content only when surrounding traffic does not relink it; the recipient mailbox key and traffic metadata remain visible.
