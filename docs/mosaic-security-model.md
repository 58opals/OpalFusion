...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.4`](mosaic-mainnet-alpha-profile.md) slice freezes exact 23-slot material, grouped balance, purpose-separated blind authorization, acknowledgements, authenticated anonymous admission, signature sets, and complete-transaction assembly. Its admission ledger binds accepted material to one attempt, generation, material, round, roster, phase, sender sequence, mailbox, authorization token, and input index; exact duplicates are idempotent and semantic rejection consumes no replay state. Alpha.4 deliberately does not prove that an authorized component opens a contributor-tagged grouped commitment, so a malicious contributor can spend an accounted token on another valid balanced component or cause an unattributable abort. Honest contributors still verify exact local inclusion, and OpalBase remains the wallet authority. The `nostr-tor/0-opal-mainnet-alpha.5` adapter authenticates regular NIP-59 gift wraps against one immutable recipient capability, seals complete control and anonymous publication batches, requires two accepted acknowledgements across three injected Tor-only routes, and starts three globally distinct subscriptions per mailbox while preserving source order. Bootstrap.1 authenticates the blind-authorized public mailbox assignment and complete roster consensus barrier before that capability exists, while retaining every anonymous recipient private key on the conductor side of the protocol. One attempt-bound admission journal records exact accepted gift-wrap bytes, original acceptance times, and semantic replay facts before coordinator effects; a separate publication journal records complete batches, permits, attempts, acknowledgements, and terminal outcomes before their effects. The macOS-only private-alpha recovery owner validates bounded canonical snapshots, reconstructs the exact formation or post-manifest coordinator, re-authenticates ordered admission before routes, resumes only persisted publication, and revalidates write-ahead abort or completion on a zero-route terminal path. Missing or inconsistent recovery fails closed. The application owns actual durable storage, cataloging, rollback/deletion detection, cross-process exclusion, recipient and authorization-key generation and custody, authenticated persistence, authoritative route policy, concrete Tor-only adapters, and deployment evidence. The generic mainnet driver remains disabled and no public Mosaic session exists. None of these package boundaries is an anonymity, accountability, or readiness claim.
...
The internal anonymous-batch publisher accepts only a bridge-minted purpose and exact material binding, withholds every sealed wrap until complete route preflight, and awaits a separate injected permit immediately before each recipient's first publication; restored continuation reuses only an already recorded permit. The permit proves only caller authorization at the recorded moment, not timing privacy; connection-object uniqueness prevents accidental in-process reuse, not shared infrastructure or Tor circuits; and two accepted acknowledgements are not durable delivery evidence.

The internal contributor transport bridge binds one validated contributor bootstrap, complete manifest, control recipient allocation, inbound control capability, and lease-backed material before connecting the coordinator's ordered publication callbacks to either publisher. Matching the inbound capability's verification key prevents a caller from substituting another roster mailbox, while the attempt transport owner exclusively provisions the role-complete inbound runtime and owns its single claim. Its nonblocking stop request is safe inside an injected callback; relay publication is cancelled, material construction is allowed to return across the wallet boundary, and an external lifecycle owner may separately await terminal drain. The bridge neither authenticates how the allocation was distributed, starts or supervises fan-in, nor automatically receives fan-in source-loss notification. Any binding, ordering, cancellation, or publication ambiguity terminalizes the bridge; an in-place bridge retry requires fresh attempt-scoped transport and material, while the separate outbound journal may continue only bytes and permits already recorded before that failure.
...
| One operator controls several configured relays | Signed messages, relay replication, and three distinct app-reviewed registry-label digests preserve validity and improve configured diversity | Registry labels and endpoint count do not prove operator, jurisdiction, infrastructure, network-path, or Tor-circuit diversity |
...
Mosaic uses proof of work and oversubscription only as throttles. The accepted private-deployment.1 supplement selects 20 leading zero bits after an Apple M1 Max measurement, but that device-specific result is neither an adversarial cost model nor a security level. The [2026-08-21 semantic decision](mosaic-private-deployment-decision-2026-08-21.md) explicitly accepts that limited posture for the private alpha. A well-funded adversary may still control most or all selected identities, become conductor, or repeatedly abort attempts. No documentation or wallet UI may imply otherwise.
...
Nostr provides replicated signed events and convenient relay discovery; it does not provide network anonymity. NIP-44 explicitly notes metadata, forward-secrecy, post-compromise, and IP-observation limits. The separately versioned accepted [private-deployment.1 supplement](mosaic-mainnet-alpha-private-deployment.md) fixes ephemeral discovery and pre-manifest kinds, exact signer authority, canonical relay selection, deadlines, abort, and completion without changing the post-manifest mapping. Those events expose discovery or control identities and timing to selected relays; they are coordination traffic, not anonymous traffic. The mainnet-alpha.5 adapter fixes an 8,192-byte padded kind-78 rumor, regular kind-1059 delivery, exact tags, cover-timestamp bounds, sender/recipient authority, and signed-wrapper replay identity. Bootstrap.1 authenticates a public-key-only mailbox distribution, an unlabeled commitment set, and complete roster acknowledgements without putting an anonymous recipient private key on the wire. Fan-in preserves each route's order and forwards signed copies through one serialized ingress. After authentication, coordinators stage semantic admission and the injected admission journal records exact accepted gift-wrap bytes, original acceptance time, and one semantic replay fact before any resulting effect; later relay copies append nothing. Recovery derives the ordered replay solely from that authenticated snapshot, re-authenticates each retained event with the bound mailbox capability, and refuses partial, reordered, extra, or semantically altered state before normal route provisioning. Distinct injected connection objects prevent accidental reuse but do not prove independent Tor circuits. OpalFusion does not generate or custody recipient keys or prove concrete Tor routing; the application must supply those capabilities and evidence, and local adapter evidence cannot prove external circuit independence. A gift wrap hides its authenticated sender from relay-visible content only when surrounding traffic does not relink it, and the recipient mailbox key and traffic metadata remain visible.
...
The anonymous-batch publisher applies the same preflight, sibling-cancellation, and closure limits to one bridge-sealed anonymous purpose batch. Its publication-permit seam remains an injected policy authority rather than a scheduler, privacy proof, or deadline, but each granted permit is write-ahead recorded before route use. That record proves only the caller-supplied journal transition; it does not prove a durable backend, timing privacy, route independence, relay delivery, or complete runtime recovery.
...
For the private deployment, a recovered signing intent without durable signed bytes must abort and release rather than re-sign. A recovered locally signed state must use the exact durable bytes to finish idempotent commit. An ambiguous broadcast intent must reconcile exact transaction presence before any new dispatch, and terminal material erasure must wait for the exact wallet and chain disposition. These are required recovery semantics, not evidence that the current application implements them.
...
These are pre-canary requirements, not release-closeout suggestions. Independent review, parser fuzzing, multi-device reliability, incident response, the protocol-disable mechanism, and integrated recovery evidence MUST be complete before any bounded-value mainnet canary. A canary cannot substitute for a missing review or be used to broaden the safe claims above. Integrating applications must establish evidence for these requirements without weakening them.
