...
The implemented [`Mosaic/0-opal.1`](mosaic-v0-profile.md) slice is a chipnet-only deterministic conformance foundation. The additive [`Mosaic/0-opal-mainnet-alpha.2`](mosaic-mainnet-alpha-profile.md) slice freezes deterministic role, manifest, PlayerCommit, authorization-response-set, complete contributor acknowledgement-set, aggregate, control-envelope, transcript, BCH-signature-set, and complete-transaction contracts, but remains runtime-disabled and rejects legacy aggregate host markers. Its admission-only ledger binds admitted control and anonymous-component documents through transcript agreement to one attempt, generation, round, roster, local role, roster-derived zero sequence epoch, and monotonic externally validated phase mirror; it cannot advance the runtime or admit BCH signing and completion. An internal profile-neutral coordinator proves in-process host-effect ordering, including late-lease release, the no-release boundary after signing may have begun, exact complete-transaction commit, and recovery-required outcomes against injected seams. Neither profile proves a concrete Tor route, commitment-opening/component linkage, previous-output-backed wallet ownership and value validation in the runnable composition, anonymous BCH-signature authorization, mailbox integration, production coordinator composition, durable recovery, or independent deployment review. These boundaries therefore do not satisfy the remaining transport, cryptographic-review, interoperability, mainnet-execution, or privacy release gates below.
