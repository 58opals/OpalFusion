...
- [`Mosaic/0-opal-mainnet-alpha.4`](mosaic-mainnet-alpha-profile.md) is a separate additive contract profile. It freezes mainnet identity, exact final-size fee allocation, contributor-local salted material, grouped Pedersen balance, two purpose-separated component-bound blind authorizations, dual-vector response sets, complete contributor acknowledgements, post-admission control sequencing, component mailbox sequence zero, BCH-signature mailbox sequence one, signature sets, and complete-transaction assembly. Its generic transport-facing runtime driver, live transport, production wallet composition, and broadcast remain fail closed.
