...
- [`Mosaic/0-opal-mainnet-alpha.4`](mosaic-mainnet-alpha-profile.md) is a separate additive contract profile. It freezes mainnet identity, exact final-size fee allocation, contributor-local salted material, grouped Pedersen balance, two purpose-separated component-bound blind authorizations, dual-vector response sets, complete contributor acknowledgements, post-admission control sequencing, component mailbox sequence zero, BCH-signature mailbox sequence one, signature sets, and complete-transaction assembly. Its alpha.5 transport mapping now has a one-shot exact-three-route/two-acknowledgement publisher over injected Tor-only capabilities, while the generic transport-facing runtime driver, inbound fan-in, concrete Tor transport, production wallet composition, and broadcast remain fail closed.
...
The generic implementation includes a strict internal NIP-59 rumor, seal, and regular/ephemeral gift-wrap structural layer. The additive mainnet contract separately selects `nostr-tor/0-opal-mainnet-alpha.5` for post-manifest traffic: unsigned normal kind-78 rumors, exact application and recipient tags, fixed 8,192-byte padded content, regular kind-1059 delivery, cover timestamps bounded by the signed manifest phase start and rumor time, signed-wrapper identity for anonymous transport replay, exact allocation ceilings, and exactly three relays with two accepted acknowledgements. This does not define discovery or pre-manifest event mappings, endpoint canonicalization, NIP-42, proof of work, relay persistence, reconnect, or Tor execution. Those remain interoperability and deployment blockers rather than implementation-defined freedoms; the `nostr-tor/1-draft.1` profile remains unresolved.
