...
- [`Mosaic/0-opal-mainnet-alpha.3`](mosaic-mainnet-alpha-profile.md) is a separate additive contract profile. Its mainnet genesis, exact final-size fee allocation, grouped Pedersen balance, authorization-response set, complete contributor acknowledgement set, and post-admission sequence epoch are frozen for deterministic implementation work, but its runtime driver, live transport, wallet execution, and broadcast remain fail closed.
...
- The checked-in mainnet-alpha admission ledger enforces exact attempt, generation, round, roster, publisher, roster-derived zero sequence epochs, one roster-wide conductor response stream, immediate externally validated phase ordering, complete PlayerCommit and authorization-response-set collection, exact request-to-response binding, contributor-local blind-response finalization, one-time anonymous sender and mailbox identities, exact complete acknowledgement-set publication, and terminal rules through transcript agreement. A paired internal runtime session owns the ledger and the validated local attempt, translates admitted documents through transcript agreement, requires an externally sealed reservation-publication validation carrying the exact lease and PlayerCommit binding, and rejects a conductor commitment set that omits any local commitment. A narrow reservation coordinator additionally requires that validation to match the actual host-returned lease, rejects non-fresh runtime state, and releases a late lease when cancellation wins; it does not prove lease-to-PlayerCommit derivation. An internal signing-request builder requires the sealed publication, exact local transcript inclusion, every contributor's acknowledgement of one transcript root, and resolver-minted ordered previous-output validation before constructing a host request. It does not invoke BCH signing, convert acknowledgements into reducer signing eligibility, or enable executable runtime composition.
