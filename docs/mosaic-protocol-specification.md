...
- The checked-in mainnet-alpha admission ledger enforces exact attempt, generation, round, roster, publisher, explicit sequence-baseline, immediate externally validated phase-ordering, complete PlayerCommit collection, one-time anonymous sender and mailbox identities, conductor-local individual-acknowledgement collection, and terminal rules through transcript agreement. It is admission-only, cannot authorize a reducer transition, exposes no conversion to runtime inputs, and deliberately terminates before BCH signing while the complete acknowledgement-set contract remains unfrozen.
