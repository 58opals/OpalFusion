...
- [`Mosaic/0-opal-mainnet-alpha.4`](mosaic-mainnet-alpha-profile.md) is a separate additive contract profile. It freezes mainnet identity, exact final-size fee allocation, contributor-local salted material, grouped Pedersen balance, two purpose-separated component-bound blind authorizations, dual-vector response sets, complete contributor acknowledgements, post-admission control sequencing, component mailbox sequence zero, BCH-signature mailbox sequence one, signature sets, and complete-transaction assembly. Its alpha.5 transport mapping now has attempt-scoped recipient lookup, a sender-global zero-origin control-publication bridge that creates a recipient-bound gift wrap for every roster peer, an exact-material-bound contributor bridge that creates anonymous component and BCH-signature wraps with distinct sequence-zero and sequence-one sender identities on the same mailboxes, a one-shot exact-three-route/two-acknowledgement publisher, bounded control and permit-gated anonymous batch publishers, and a bounded multi-recipient fan-in over externally provisioned three-route Tor-only mailbox groups. Both bridges' whole-batch callbacks, publisher routes, publication permits, and fan-in capabilities remain injected rather than a production network composition, and the fan-in forwards every signed EVENT copy without cross-relay merging, so the generic transport-facing runtime driver, complete production mailbox allocation, durable digest-and-sequence duplicate merging and replay, concrete Tor transport, durable key ownership, production wallet composition, and broadcast remain fail closed.
...
- The checked-in internal control-batch publisher accepts only a bridge-minted batch with the exact bootstrap context, complete roster recipient allocation, and manifest relay selection; obtains one complete recipient-event-identity route allocation before publication; rejects connection reuse across recipients; delegates each gift wrap to one exact-three-route/two-accepted-ACK publisher; and drains every route before returning. It does not provision endpoints or Tor circuits, persist acknowledgements or delivery state, retry, or admit semantic loopback.
...
- The checked-in anonymous-batch publisher accepts only the bridge-minted purpose and material binding, requests every route using recipient event identities before publication, rejects allocation mismatch and batch-wide connection reuse, awaits one injected publication permit per recipient, delegates every sealed wrap to the exact-three-route/two-accepted-ACK publisher, and drains all routes. The permit and route providers remain external authorities; this value neither defines timing nor proves endpoint authority, Tor-circuit isolation, durable delivery, or live execution.
...
The generic implementation includes a strict internal NIP-59 rumor, seal, and regular/ephemeral gift-wrap structural layer. The additive mainnet contract separately selects `nostr-tor/0-opal-mainnet-alpha.5` for post-manifest traffic: unsigned normal kind-78 rumors, exact application and recipient tags, fixed 8,192-byte padded content, regular kind-1059 delivery, cover timestamps bounded by the signed manifest phase start and rumor time, signed-wrapper identity for anonymous transport replay, exact allocation ceilings, and exactly three relays with two accepted acknowledgements. The internal profile-scoped control-publication bridge owns one roster member's zero-origin sender sequence, preclaims each reservation-plus-fragment range, emits the same signed inner envelope to every roster recipient through distinct gift wraps, and terminalizes rather than reusing a range after cancellation, concurrent use, timestamp/signing failure, or partial batch handoff. The internal profile-scoped fan-in validates one shared manifest relay selection, requires exactly one control mailbox and permits only conductor-side anonymous mailbox groups up to roster capacity, assigns each mailbox three globally distinct injected connections and subscription identifiers, starts every subscription before consuming into one ingress/runtime authority, serializes every received signed EVENT copy through a bounded queue while preserving each route's source order, ignores EOSE and NOTICE, and treats any selected-source loss as terminal. These boundaries deliberately do not satisfy the normative durable cross-relay digest-and-sequence merge requirement above: complete production mailbox allocation, authenticated recipient-key distribution, durable duplicate merging, replay and sequence state, and reconnect remain absent. This transport slice also does not define discovery or pre-manifest event mappings, endpoint canonicalization, NIP-42, proof of work, relay persistence, concrete timing policy, or Tor execution. Those remain interoperability and deployment blockers rather than implementation-defined freedoms; the `nostr-tor/1-draft.1` profile remains unresolved.

The control-batch publisher supplies the bounded roster-wide handoff omitted from the bridge callback: it requires the exact batch context and recipient mapping, requests all route groups by recipient event identity before exposing an event to a relay session, rejects batch-global connection reuse, cancels sibling publications on failure, and drains every delegated publisher. The route owner still supplies authoritative endpoint-to-capability-bound Tor routes and all persistence, retry, and recovery policy.

The anonymous-batch publisher supplies the equivalent bounded handoff for one bridge-minted component or local-signature batch. It preflights every recipient route group before exposing a wrap, then requires a caller-owned per-recipient publication permit immediately before relay delegation. The provider must supply fresh anonymous route capabilities; the publisher's in-process object checks do not define timing policy or establish Tor-circuit independence.
