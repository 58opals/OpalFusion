...
| Mosaic | `.mosaic(configuration)` | `Mosaic/1-draft.1`, chipnet-only `Mosaic/0-opal.1`, or contract-only `Mosaic/0-opal-mainnet-alpha.2` | Deterministic foundations with bounded internal adapters; only Opal v0 reaches the internal runtime driver; no live session |
