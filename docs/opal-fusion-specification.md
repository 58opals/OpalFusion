...
Status: Draft architecture contract. CashFusion has a live pilot implementation. Mosaic has internal aggregate and peer-local attempt cores, an internal ordered host coordinator, the chipnet-only `Mosaic/0-opal.1` conformance profile, and the additive `Mosaic/0-opal-mainnet-alpha.3` deterministic contract profile. The coordinator proves late-reservation release, the no-release boundary after signing may have begun, exact complete-transaction commit, and recovery-required ordering against injected seams. The mainnet-alpha profile freezes role hashing, complete manifest, PlayerCommit, deterministic fixed-overhead fee allocation, grouped Pedersen balance validation, per-contributor authorization-response sets, the complete contributor acknowledgement set, aggregate reservation and strict typed reassembly, authenticated control-envelope admission, BCH signature-set, exact complete-transaction assembly, and profile-separated authorization/transcript vectors. Its internal admission ledger enforces a roster-derived zero sequence epoch, per-sender replay, attempt- and generation-bound immediate phase synchronization, complete PlayerCommit and response-set collection, contributor-local blind-response finalization, conductor-only anonymous intake with one-time sender and mailbox identities, exact acknowledgement-set publication, and terminal behavior through transcript agreement. A paired internal mainnet-alpha runtime session translates those sealed facts into the same validated local attempt through transcript agreement, requires an externally sealed reservation-publication validation carrying the exact lease and PlayerCommit binding, and verifies every local commitment is present in the conductor set. A narrow reservation coordinator additionally accepts that validation only when it matches the actual host-returned lease, rejects non-fresh runtime state, and releases the lease when cancellation wins, while leaving material derivation and publication proof injected. A public previous-output source contract and internal mainnet transcript resolver enforce exact ordered outpoint and amount binding without authorizing signing or broadcast. An internal signing-request builder requires the sealed publication, local transcript inclusion, roster-complete acknowledgement set, and resolver-minted previous-output validation before constructing the existing host request; it does not invoke signing or enable the mainnet runtime. The runtime explicitly refuses BCH-signing and complete-transaction admission and does not enable the runtime driver. Discovery, salt/opening and privacy-preserving component linkage, production lease-to-material and PlayerCommit construction, a production previous-output source in executable runtime composition, anonymous BCH-signature authorization, production mailbox ingress, a concrete Tor route, full production coordinator composition and durable recovery, independent review, public Mosaic session execution, and mainnet broadcast remain unimplemented or explicitly gated.
...
| Mosaic | `.mosaic(configuration)` | `Mosaic/1-draft.1`, chipnet-only `Mosaic/0-opal.1`, or contract-only `Mosaic/0-opal-mainnet-alpha.3` | Deterministic foundations with bounded internal adapters; only Opal v0 reaches the internal runtime driver; no live session |
