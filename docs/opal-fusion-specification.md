...
Status: Draft architecture contract. CashFusion has a live pilot implementation. Mosaic has internal attempt/runtime cores, an ordered host coordinator, the chipnet-only `Mosaic/0-opal.1` conformance profile, and the additive `Mosaic/0-opal-mainnet-alpha.4` deterministic contract profile. Alpha.4 freezes exact contributor material, purpose-separated authorization, complete acknowledgement, authenticated mailbox admission, canonical signatures, and complete-transaction contracts. Its admission ledger enforces attempt, generation, material, roster, phase, replay, mailbox, token, and input-index rules; role executors prove previous-output-backed assembly, signing, wallet disposition, and exact commit through injected seams. The `nostr-tor/0-opal-mainnet-alpha.5` mapping freezes kind-78 rumors, fixed padding, regular NIP-59 gift wraps, recipient binding, cover timestamps, signed-wrapper replay identity, and three-relay/two-acceptance publication. Ingress selects one immutable recipient capability before authentication, publication bridges seal exact control and anonymous batches, and fan-in starts three globally distinct injected subscriptions per mailbox while preserving source order and failing closed on source loss. One attempt-bound journal sends only semantically accepted digest/sequence and wrapper/mailbox replay facts through a caller-supplied append boundary before coordinator effects; identical relay copies append once, rejected traffic remains unconsumed, and restored nonempty state refuses a fresh runtime. The application layer owns production live-material persistence, recipient and authorization-key custody, authoritative mailbox and route policy, concrete Tor-only adapters, durable acknowledgement and replay state, reconnect cleanup, supervised session execution, deployment evidence, and broadcast approval through OpalBase without a direct Wallet-to-OpalFusion dependency. The generic transport-facing mainnet runtime driver remains disabled. The non-normative progress record owns the current status of those obligations.
