...
Status: Draft architecture contract. CashFusion has a live pilot implementation. Mosaic has internal aggregate and peer-local attempt cores, an internal ordered host coordinator, the chipnet-only `Mosaic/0-opal.1` conformance profile, and the additive `Mosaic/0-opal-mainnet-alpha.1` deterministic contract profile. The coordinator proves late-reservation release, the no-release boundary after signing may have begun, exact complete-transaction commit, and recovery-required ordering against injected seams. The mainnet-alpha profile freezes role hashing, complete manifest, PlayerCommit, aggregate reservation and strict typed reassembly, authenticated control-envelope admission, individual pre-sign acknowledgement, BCH signature-set, exact complete-transaction assembly, and profile-separated authorization/transcript vectors. Its internal admission ledger enforces explicit per-roster sequence baselines, per-sender replay, attempt- and generation-bound immediate phase synchronization, complete PlayerCommit collection, conductor-only anonymous intake with one-time sender and mailbox identities, conductor-local individual-acknowledgement collection, and terminal behavior through transcript agreement. It explicitly refuses BCH-signing and complete-transaction admission, does not enable its runtime driver, and produces no runtime facts. Discovery, commitment-opening/component linkage, authorization-response and complete acknowledgement-set contracts, previous-output resolution, anonymous BCH-signature authorization, mailbox-to-runtime integration, a concrete Tor route, production coordinator composition and durable recovery, independent review, public Mosaic session execution, and mainnet broadcast remain unimplemented or explicitly gated.
