...
The current Mosaic checks cover the attempt, role, roster, reservation, transcript, terminal, retry, generation, deterministic simulation, Opal v0 authorization, canonical component, aggregate, relay-validation, strict replay, runtime-driver ordering, structural wallet-host boundaries, and the ordered host coordinator. Coordinator tests deterministically suspend reservation, publication, signing-request construction, and signing to prove both sides of the release-versus-publication/signing gates, late-lease release, release-before-terminal ordering, no automatic release after signing may have begun, terminal local-result source closure, exact complete-transaction commit instead of the legacy commit overload, and recovery-required release or commit failure. Mainnet-alpha coverage additionally pins the profile and genesis identifiers, canonical 6–8-contributor fixed-overhead shares totaling ten satoshis, exact component/final-fee arithmetic, profile-bound grouped commitments, manifest-bound Pedersen sum validation, role documents, complete manifest and locally agreed proposal context, PlayerCommit, per-contributor authorization-response sets, the complete contributor acknowledgement set, profile-separated authorization and transcript digests, kind-bounded strict typed reassembly, authenticated outer-identity/control publisher/round/phase/expiry admission, anonymous sender and recipient binding, signature-set wire ordering, component-committed and resolved previous-output amount equality, P2PKH signature verification, the 100-byte unlocking script, exact unsigned-body matching, the 184-nonblank-component limit, and the complete-transaction payload. The previous-output contract suite additionally proves exact request shape, ordered outpoint and amount binding, source-asserted locking-script and token-state carriage, token-presence rejection, output substitution and reordering rejection, sanitized source failure, cancellation precedence after source I/O, and mainnet-only transcript resolution without signing authority. The signing-request-builder suite requires the exact sealed lease and PlayerCommit publication, local transcript-inclusion validation, complete contributor acknowledgement set for the same round and root, and resolver-minted previous-output validation; it proves canonical multi-input index mapping, remote-key absence, exact local amount, script, key, output-multiset, and fee-share checks without invoking signing. Admission-ledger tests separately add exact attempt/generation routing, roster-derived zero sequence epochs, per-sender concurrent fragment runs, same-sender interleaving rejection, one roster-wide conductor response stream, 6–8-contributor PlayerCommit and response-set barriers, exact request-to-response binding, manifest-key binding and distinct replay identities for all 23 locally finalized responses, exact manifest-core and transcript-root binding, local conductor-only authorized anonymous-component intake with one-time sender and mailbox identities, conductor-local acknowledgement collection and exact portable-set publication, attempt- and generation-bound phase synchronization, explicit BCH-signing closure, terminal absorption, in-place retry rejection, and stale-input rejection by a distinct fresh instance. Paired mainnet-alpha runtime-session tests add 6–8-contributor common construction, exact ledger-to-local-attempt translation through transcript agreement, an externally validated reservation-publication binding seam, field-isolated substitution rejection, exact local-commitment inclusion, conductor non-contribution, transcript equality, release-before-terminal failure ordering, cancellation, terminal absorption, and in-place retry rejection. The profile-scoped reservation-coordinator suite binds the actual host-returned lease to that sealed validation, rejects direct token injection, non-fresh runtime state, and mismatched complete leases, validates every frozen request field and an exact caller-owned expiration, drains a late lease or claimed publication before cancellation, rejects a conductor before wallet work, and requires recovery after release failure. It does not select the manifest-deadline-to-lease-expiry mapping or prove production PlayerCommit derivation. The complete acknowledgement set remains transcript-bound and does not emit BCH-signing eligibility. These admission and bridge checks do not claim an authoritative production previous-output adapter or complete-transaction admission; cryptographic transaction checks belong to the separate profile-contract and coordinator suites. Cross-profile, malformed canonical bytes, foreign round, wrong phase or publisher, wrong excess share or grouped Pedersen balance, local PlayerCommit-digest mismatch, foreign authorization key, repeated authorization replay identity, invalid blind response, copied-root transaction substitution, legacy mainnet host markers, driver initialization, wrong reservation-publication binding, wrong acknowledgement round, root, or roster, local commitment substitution, wrong outpoint, amount, or locking script, and invalid BCH signature cases fail closed.

The transcript-binding contract supports Opal v0 and mainnet-alpha, while `RuntimeSessionDriver` supports only Opal v0 and rejects draft and mainnet-alpha. The paired mainnet-alpha runtime session is an internal deterministic reducer bridge, not an executable driver or host composition. The coordinator uses injected request, publication, inclusion, signature, and complete-transaction providers; these checks do not prove their production implementations, discovery, salt/opening and privacy-preserving component-linkage construction, production lease-to-material or local-inclusion validation, live reservation and PlayerCommit derivation provenance, previous-output resolution by a live host, anonymous BCH-signature authorization, production mailbox ingress, concrete Tor routing, durable crash recovery, independent protocol or side-channel review, mailbox unlinkability, anonymous transport, blame cryptography, broadcast safety, or live mainnet execution.
