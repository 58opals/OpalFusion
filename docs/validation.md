...
Focused filters isolate one layer, but they are not uniformly fast. `MosaicMainnetAlphaPostManifestContributorTransportBridgeValidator` and `MosaicMainnetAlphaMinimumRosterCompositionValidator` are the fast contributor lifecycle and composition loops; neither constructs RSA signing keys. `MosaicMainnetAlphaPostManifestContributorTransportBridgeConformanceValidator`, together with every suite named by the wrapper's `MOSAIC_RSA_DEPENDENT_FILTER`, is deterministic but intentionally slow and serialized. None of the focused filters require coordinator credentials, wallet secrets, live UTXOs, Tor, or funded test material.
