...
Run the full deterministic local suite for normal repo work through the repository wrapper. It keeps the Security.framework-backed Mosaic authorization suites and the reconnect-timing client suite in isolated serialized processes while leaving the remaining deterministic tests in the normal pool:
