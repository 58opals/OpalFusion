...
The current Mosaic checks cover roster and phase invariants, manifest and transcript gates, terminal and retry behavior, peer-local generation guards, deterministic 7–9-peer semantic simulation, Section 8 canonical primitives, the Opal v0 profile selector, transcript-to-transaction binding, the 23-slot all-contributor issuance barrier and duplicate cache, token replay identifiers, fixed-size inner-envelope boundaries, exact conformance tags and event kinds, strict sequence progression, and structural wallet-host bindings. They consume already-validated facts and do not prove discovery, complete named wire-message layouts, production blind cryptography, mailbox unlinkability, anonymous transport, blame cryptography, or live execution.
