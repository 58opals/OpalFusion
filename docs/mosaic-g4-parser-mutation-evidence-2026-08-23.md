...
| Private test-only harness | `f2ecfe8638434ad2b5e8c941edef624ce9132532` | First-party deterministic mutation helper, expanded focused parser campaign, and test-fixture visibility change |
...
`git diff --name-only public/develop..f2ecfe8638434ad2b5e8c941edef624ce9132532 -- Sources Package.swift Package.resolved` is empty. This private checkpoint does not promote a new OpalFusion runtime or require a Wallet dependency repin.
...
| Typed private-deployment Nostr mapping | Availability beacon, candidate-set acknowledgement, candidate admission, role commitment, role reveal, manifest proposal, manifest signature, contributor nonce allocation, abort, and completion events |
...
| Durable post-manifest journals | One admission snapshot containing control and anonymous records, plus one drained publication snapshot containing prepared, attempted, acknowledged, and completed records |

The current campaign therefore contains 70 positive seeds across ten focused test bodies in five suites: 8 Nostr, padded-envelope, and relay-frame seeds; 13 Opal v0 seeds; 15 post-manifest wire seeds; 17 private-deployment document seeds; 10 typed pre-manifest and completion Nostr seeds; 1 runtime-recovery seed; 4 private event and terminal-recovery seeds; and 2 durable journal seeds. Existing targeted positive, negative, and deterministic parser-mutation tests elsewhere in the package remain valid evidence, but they are not relabeled as part of this campaign.

A simple source inventory finds 93 declarations containing `func decode` under `Sources/OpalFusion/Mosaic`, but that spelling-based number includes helpers and misses byte entrypoints named `load`, `restore`, `open`, or `validate`. The new [parser-root registry](mosaic-g4-parser-root-registry-2026-08-23.md) instead freezes the 52 production files that directly invoke the canonical or JSON decoder substrate and provides a machine freshness check. That file set is still not a completed root map: overloaded functions, context-specific variants, and private helpers can share one externally reachable root. The 70 seeds are not claimed as a coverage fraction.
...
| Durable publication recovery parser | Passed in 6.311 seconds | New affected selector |
| Nine typed pre-manifest Nostr parsers | Passed in 19.873 seconds | New affected selector |
| Typed completion Nostr parser | Passed in 19.812 seconds | New isolated affected selector |

All ten selected test bodies therefore have passing evidence at `f2ecfe8638434ad2b5e8c941edef624ce9132532`. They were not rerun as one final combined command, so no combined elapsed time or single-run `10/10` result is claimed. This follows the cost-bounded validation policy: preserve still-valid passes and rerun only a failed or subsequently changed selector. The static registry command `scripts/check-mosaic-g4-parser-root-registry.sh` also passed for its frozen 52-file implementation surface.
