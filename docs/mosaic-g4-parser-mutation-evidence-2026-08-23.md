...
Status: `partial`. Bounded first-party deterministic mutation and positive-variant campaigns now cover all 52 registered low-level OpalFusion parser files. Composite and cross-repository reconciliation remain before the G4 every-parser requirement can close.
...
This slice does not satisfy “every parser.” All 52 registered low-level OpalFusion implementation files and their reviewed recovery discriminants are covered. Known OpalFusion work is now composite reconciliation: transport consensus-envelope opening, publication restoration, and any other parent entrypoint that adds authority, time, route, or binding validation around an already-mapped low-level parser. Wallet and OpalBase add ten passing cross-repository seeds across four test bodies, while relevant OpalCrypto inputs are mapped transitively through existing deterministic matrices; final application-owned root reconciliation remains open. Private helper decoders covered transitively through a root still need an explicit mapping rather than another redundant seed.

The next package slice should reconcile composite parent roots against existing focused evidence and add a new seed only where a parent imposes materially distinct untrusted-byte validation. Every parser still requires explicit fuzz-coverage evidence; if the bounded deterministic campaign cannot supply that evidence, the project must add a first-party coverage-guided harness or another defensible first-party fuzz lane. Adding an external production dependency is not authorized or necessary.
