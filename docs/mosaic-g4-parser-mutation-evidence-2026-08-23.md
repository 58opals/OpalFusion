...
Status: `partial`. A bounded first-party deterministic mutation campaign now covers all 49 fully dispositioned low-level OpalFusion parser files, including the formerly held transport-bootstrap family. Three recovery-variant files plus composite and cross-repository reconciliation remain before the G4 every-parser requirement can close.
...
| Private test-only harness | `5e237bfcb1d8402ba8f1f86b4d8260b18fa23f49` | First-party deterministic mutation helper, digest-pinned transport fixture, and focused parser campaigns |
...
`git diff --name-only public/develop..5e237bfcb1d8402ba8f1f86b4d8260b18fa23f49 -- Sources Package.swift Package.resolved` is empty. This private checkpoint does not promote a new OpalFusion runtime or require a Wallet dependency repin.
...
The current OpalFusion campaign therefore contains 85 positive seeds across 21 focused mutation-test bodies: the prior 75 seeds across twelve bodies plus ten transport-bootstrap seeds across nine bodies. Existing targeted positive, negative, and deterministic parser-mutation tests elsewhere in the package remain valid evidence, but they are not relabeled as part of this campaign.

A simple source inventory finds 93 declarations containing `func decode` under `Sources/OpalFusion/Mosaic`, but that spelling-based number includes helpers and misses byte entrypoints named `load`, `restore`, `open`, or `validate`. The [parser-root registry](mosaic-g4-parser-root-registry-2026-08-23.md) freezes the 52 production files that directly invoke the canonical or JSON decoder substrate, and the machine-checked [root map](mosaic-g4-parser-root-map-2026-08-23.txt) assigns them 49 covered, three partial-variant, and zero closure-budget dispositions. That map is not cross-repository closure: overloaded functions, context-specific variants, composite parents, and raw cryptographic or application envelopes still require reviewed accounting. The 85 seeds are not claimed as a coverage fraction.
...
All 21 selected mutation-test bodies therefore have preserved or new passing evidence at private test harness `5e237bfcb1d8402ba8f1f86b4d8260b18fa23f49`. They were not rerun as one final combined command, so no combined elapsed time or single-run `21/21` result is claimed. This follows the cost-bounded validation policy: preserve still-valid passes and rerun only a failed or subsequently changed selector. After adding the optional mutation cap, the unchanged full-campaign default path was rechecked through `mutateNostrEventAndRelayParsers` and passed in 0.625 seconds. The static registry command `scripts/check-mosaic-g4-parser-root-registry.sh` passes for its frozen 52-file implementation surface and exact 49-covered / 3-partial-variant / 0-closure-budget root-map disposition.
...
This slice does not satisfy “every parser.” The manifest-proposal-candidate root, validated-manifest plus initialized-journal recovery discriminants, control and anonymous NIP-59 composite opens, application-content extraction, publication-permit journal record, and all eight registered transport-bootstrap implementation files are now covered. Known OpalFusion work still includes positive pre-manifest abort, publication formation and terminal, and authorized terminal recovery discriminants, plus composite transport consensus-envelope opening and publication restoration reconciliation. Wallet and OpalBase add ten passing cross-repository seeds across four test bodies, while relevant OpalCrypto inputs are mapped transitively through existing deterministic matrices; final application-owned root reconciliation remains open. Private helper decoders covered transitively through a root still need an explicit mapping rather than another redundant seed.
